...
2. **5c 逾期待查證清算**——照該檔「5c. 逾期待查證清算」執行，**Lane A（本輪額度 10）＋Lane B（本輪額度 5）**（`check_pending_markers.py --queue` 已內建兩條分流；Lane A 多數可免 web，但**雲端 egress 封鎖時整個 5c 跳過**，本步本來就在本機執行）。務必照 5c 第 5 步做結案回掃，並把輸出的「📊 產消對帳」與末尾「⚠️ 舊語法盲區」一併抄進回報
