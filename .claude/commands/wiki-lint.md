...
#### 6h. 品質指標月報 `[加入: 2026-07-05]`

**僅在每月第一次執行 `/wiki-lint` 時執行**（判斷方式：`wiki/log.md` 中本月尚無 `Lint` 記錄）；其餘週次跳過，輸出「非本月首次 lint，跳過品質指標月報」。
...
輸出：
```
📊 品質指標月報（YYYY-MM，近 7 天 / 14 天窗口）：
  ref 覆蓋率：XX%（閾值 80%）→ ✅ / ⚠️（缺 ref 日期：…）
  採用驗證率：⏳→⚡/✅ 共 N 條中 M 條達成（XX%，僅供判讀）
```
...
- 品質指標月報（6h）：（本月首次執行才有；否則寫「非本月首次 lint，跳過」）
  - ref 覆蓋率：XX%（閾值 80%），缺 ref 日期：（列出或「無」）
  - 採用驗證率：N 條中 M 條達成（XX%，僅供判讀）
