...
2. 對每個條目 `Grep` 近 4 週 `news/*.md` 有無提及（用功能名或其關鍵旗標／版本號，不用整串括號說明）
3. 零命中 → 熱度 −1 格，**同步對應 `entities/` 頁的「熱度與試用價值」表**——單邊下修是矛盾的來源
4. **不降的例外**：狀態為「⏰ 倒數中」，或本輪熱度／試用價值有其他變動者
5. 回報：`熱度降溫：檢查 N 條，降 M 條（列出 條目名 舊→新），同步 entities 頁 M 處`
...
1. **盤點＋排序**：先跑**完整報告**再跑佇列——
   ...
   佇列部分：`--queue`，輸出即逾期佇列，排序已內建（訊欄優先→逾期天數降序），上限 5 筆即本輪處理額度（`[改版: 2026-08-20]` 原為 10；改由 `/weekly` 每週執行後，寧可每週穩定 5 筆，也不要一次 10 筆卻整月只跑一次），不需另外手動排序。**輸出末尾的「⚠️ 舊語法盲區」不是裝飾** `[加入: 2026-08-20]`——那是佇列撈不到的存量與其頁面分佈；「總逾期數 0」只代表新語法那半乾淨了。把盲區筆數與前三頁抄進本步回報，並在 3g 派工時優先指定那幾頁回填
