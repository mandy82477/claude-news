...
1. **盤點＋排序**：`python scripts/check_pending_markers.py --queue`，輸出即逾期佇列，排序已內建（訊欄優先→逾期天數降序），上限 5 筆即本輪處理額度（`[改版: 2026-08-20]` 原為 10；改由 `/weekly` 每週執行後，寧可每週穩定 5 筆，也不要一次 10 筆卻整月只跑一次），不需另外手動排序。**輸出末尾的「⚠️ 舊語法盲區」不是裝飾** `[加入: 2026-08-20]`——那是佇列撈不到的存量與其頁面分佈；「總逾期數 0」只代表新語法那半乾淨了。把盲區筆數與前三頁抄進本步回報，並在 3g 派工時優先指定那幾頁回填
