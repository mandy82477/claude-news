...
1. **盤點＋排序**：先跑**完整報告**再跑佇列——
   ...
   - **Lane A（本輪額度 10）**：記者已標 `訊`，代表日報裡已有後續證據——結案只需讀該日日報改寫，**不需要 web**。判斷仍需與 Lane B 同一輪一致，**雲端 egress 封鎖時整個 5c 跳過**
   ...
   **📊 產消對帳那一行必須讀，並抄進本步回報** `[加入: 2026-08-29]`：它印出「近 7 天新增 N 筆｜本輪合計額度 15 筆｜淨增/淨減」。**只看「總逾期數」看不出流量**——2026-08-29 實測近 7 天新增 24 筆、額度 15 筆，結構性淨增約 9 筆/週；正是因為改版前輸出沒有這一行，佇列 19 天內從 0 長到 51 筆而無人察覺。出現「⚠️ 產出快過消費」時，在回報中提出處置建議（提高額度／記者端提高標記門檻），不得只抄數字。
