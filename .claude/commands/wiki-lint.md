...
**月度蒸餾（記者成長迴路）`[加入: 2026-07-05]`：** 僅每月第一次 lint 執行（判斷方式同 6h 指標二：`wiki/log.md` 本月尚無 `Lint` 記錄），其餘週次輸出「非本月首次 lint，跳過月度蒸餾」。
...
- **行數預算硬約束**：目標記者規則檔（`.claude/rules/wiki-ingest-[category].md`）若已超 150 行，提案必須附帶等量的合併或刪減方案，不可淨增
