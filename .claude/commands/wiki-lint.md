...
> lint 專用回報格式的「轉知處置」欄必須與 `.claude/rules/wiki-reporter-shared.md` 的回報契約同步 `[加入: 2026-08-28]`（教訓見本檔「沿革」2026-08-28 A）。
...
> 行為層抽驗的依據：**連續滿分與抓不到問題是同一枚硬幣**；對照全域 `REVIEW-PRINCIPLES.md` A1（不信回報、獨立重驗）。`[加入: 2026-08-28]`（教訓見本檔「沿革」2026-08-28 B）
...
1. **盤點＋排序**：先跑**完整報告**再跑佇列——
   ...
   > 為何分流：一個額度混用兩種成本結構的工作，必然被便宜那種佔滿（教訓見本檔「沿革」2026-08-29 A）。
...
3. **寫回**（四選一，不得留原狀）：
   ...
   > **第四列為何不可用 `🔎`** `[加入: 2026-08-29]`：`🔎 查無官方` 的定義是「**已查官方一手來源、確認未載**」；Lane A 沒查官方，用它等於宣稱做過沒做的事。（教訓見本檔「沿革」2026-08-29 B）
...
5. **結案回掃（強制，每筆查實後立刻做，不得留到最後）** `[加入: 2026-08-20，改版: 2026-09-04]`：語意與 `.claude/rules/wiki-reporter-shared.md`「事實更正必回掃」同一套，**先入邊、後 grep**——
   ...
   > **為什麼結案一定要回掃：同步是雙向的，但機制只有單向。** 3a 判矛盾會把結論**下修**到低確信度頁面，5c 查實後**上修**時那批引用方不會自己回來。（教訓見本檔「沿革」2026-08-20）
...
> 本檔必須在掃描範圍內：**檢查者把自己排除在檢查範圍外，是這類缺陷的共同形狀。**（教訓見本檔「沿革」2026-08-28 D）
...
> 範圍刻意含 `wiki-ingest.md`（無後綴，接不到 `wiki-ingest-*.md` glob）與本檔（全庫 `[加入:]` 最密）。（教訓見本檔「沿革」2026-08-28 D）
...
- 起因見本檔「沿革」2026-09-02（另見 `wiki/log.md` 當日 Query）
...
> **分類器口徑 `[加入: 2026-08-20]`**：`dead` 只認 404/410；401 併入 `anti_bot`；逾時與 5xx 先用較長逾時重試一次，仍失敗才歸 `unverified`。**單次逾時是很弱的證據**——連續多週落在 `unverified` 才值得人工看。（分三桶與改讀檔的教訓見本檔「沿革」2026-08-20 B／C）
...
6d 問「逾 60 天的規則還吻合嗎」，從不問「還需要存在嗎、能不能只留判準」。本步是密度的消費端。（首次量測與裁決見本檔「沿革」2026-09-04）
...
> 擱置週數是 `/weekly` 步驟 0 呈報時的排序依據（使用者不讀 `wiki/log.md`）`[加入: 2026-08-28]`。（教訓見本檔「沿革」2026-08-28 E）
...
3b. **渲染層驗收（build 成功時必做）`[加入: 2026-08-28]`**：對本輪**改動最多的 2 頁**，看渲染後的結果，不是只看 markdown——
   ...
   > **為什麼不能只信 build 綠燈 `[加入: 2026-08-28]`**：`build_web.py` 只證明「資料層對」，不證明「讀者看到的東西對」；全域 `REVIEW-PRINCIPLES.md` 第 11 條明言 DOM／資料斷言不可替代眼睛驗收。（教訓見本檔「沿革」2026-08-09）
...
本節是**歷史敘事，不是待執行規則**——條文已在上方，執行時不必讀本節。存放於此的原因：條文本身已能獨立執行，敘事只在有人想問「為什麼有這條」時才需要。考古鏈為 `[加入: 日期]` → 本節 → `wiki/log.md` 同日 Query 條目。
...
**2026-09-04**（6h 規則密度）：首次量測 30 檔 4,585 行、235 個 `[加入:]` 標記、95 行教訓敘事；`wiki-lint.md` 自己 567 行 50 標記——檢查者最肥，同「檢查者把自己排除在檢查範圍外」形狀。密度沒有消費端，就跟 wiki 蒸餾上線前的 patterns 頁一樣單向增長。同日使用者裁決「全清」，五個候選檔一次減肥（本節即該次產物）。
