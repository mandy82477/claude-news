...
修改 `src/news_aggregator/`、`scripts/build_web.py`、`.claude/skills/news-digest/format.md`、`.claude/skills/news-digest/selection.md` 的日報格式與選材規格、或收錄門檻規則（`.claude/reporter-rules/wiki-reporter-shared.md` 互動門檻對照表）**之前與之後**執行，防止改版靜默劣化產出。依 `$ARGUMENTS` 分兩種模式。
