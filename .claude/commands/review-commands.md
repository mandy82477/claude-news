...
**終止條件：完整跑完本輪等級的所有檢查，零 ❌，才可結束。每發現一個 ❌ 立即修正，然後從頭重新開始整輪。**

---

## Step 0：改動分級（決定本輪範圍）

先確認本次改了什麼（`git diff --name-only HEAD~1` 或依對話上下文列出改動檔案與性質），對照下表取**最高**命中等級：

| 等級 | 觸發條件（任一命中即該級） | 執行範圍 |
|------|--------------------------|---------|
| **輕量** | 只在既有結構內增修內容：加表格列、加欄位說明、改文字措辭、更新日期或數值，**且**未動任何路徑引用、未動同步配對註冊表雙方的「必須一致的內容」、未動步驟結構 | 只跑「受影響檢查」：(a) 對改動檔案本身跑檢查 1a/1b；(b) 同步配對註冊表中**涉及改動檔案**的配對跑其驗證 grep；(c) 若改動新增了檔案路徑引用，驗證該路徑存在。全部用 grep/test 完成，**不逐讀未改動的檔案** |
| **結構** | 新增/刪除/改名任何 rules、commands 檔；修改任何同步配對「必須一致的內容」本身；修改回報格式欄位名；修改 command 的步驟編號或載入清單；一次改動 ≥ 3 個規則檔 | 完整四項檢查（下方檢查 1–4） |

> 分級判斷不確定時，從嚴取「結構」。輕量輪中若任何 grep 發現 ❌，修正後升級為「結構」全輪重跑（修正本身可能有連鎖影響）。

**執行成本原則：** 本指令為機械式一致性檢查，不需旗艦模型。主 session 若為旗艦模型，可將整份檢查（含本檔內容與改動清單）派工給 `model: "sonnet"` 的 agent 執行並回收報告；主 session 只驗收「零 ❌」結論。
...
逐一檢查 `.claude/commands/*.md` 及 `.claude/rules/*.md` 所有檔案（用 grep 掃描即可，只在 grep 命中需人工判讀時才 Read 該檔對應段落，不需全文逐讀）。
...
```
Step 0 分級 → 決定本輪範圍（輕量 or 結構）
do {
  輕量 → 只跑受影響檢查（Step 0 表定義）
  結構 → 執行檢查 1 → 2 → 3 → 4
  if 發現任何 ❌:
    立即修正
    升級為「結構」，重新從檢查 1 開始
