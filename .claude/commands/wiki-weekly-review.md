...
**每月執行一次**（判斷方式：`wiki/metrics.md` 的「聚焦命中率」欄**本月尚無數值** → 執行；已有數值 → 輸出「本月聚焦校準已執行，跳過」並跳過本節）。判斷產出物而非執行記錄——2026-07-16 教訓：舊判斷方式查「log.md 本月尚無週度延伸回顧記錄」，規則誕生前的回顧記錄佔用當月名額，導致連續三週誤跳過、校準從未實際執行。
