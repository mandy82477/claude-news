...
沿用 `.claude/commands/wiki-lint.md` 「步驟 2」的類別對應表（模型/功能/商業/安全政策/社群/人物 → 對應 subagent_type）。
...
使用者確認後，若牽涉多頁修改可再次平行派工對應記者執行；若牽涉新頁面建立，讀 `.claude/rules/wiki-ingest-format.md` 頁面格式模板。
