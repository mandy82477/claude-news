...
- **每則說明句必須讀得出「為什麼今天它是重點」**——跨幾個來源、當日互動數字、或發生了什麼變化；寫不出今日訊號的條目不進本區
...
**聚焦防重複 `[加入: 2026-09-04]`（適用所有正文區塊）**：已被「📌 今日聚焦」點名的條目，在所屬區塊的說明句**寫聚焦沒講的增量**（新細節、數字、脈絡），**禁止逐字或近逐字複製聚焦句**——冷讀者實測：5 分鐘裡有 1 分鐘在讀同一件事第二遍。中文說明也不得只是標題直譯，答不出「比標題多說了什麼」就重寫。
...
**檔尾 `[改版: 2026-09-04]`**：📡 來源狀態表即是檔案結尾——「選材門檻」附錄與「今日聚焦參考連結」清單皆已廢除（門檻是內規不上讀者版；聚焦連結已改行內）。若 📰 媒體報導／💰 付費方案動態等常設區塊本日因無料省略，在 📡 來源狀態表上方加一行讀者語言說明（如 `> 本日 Google News 0 則，故無媒體報導區`），讓讀者分得出「版式改了」與「今天沒料」。
