...
**網站上讀者版日期的版面 `[改版: 2026-09-12 乙-2]`**：頂部先畫 Step 1b 產出的 **📌 今日聚焦**與 **⭐ 重點話題**（前 5 則；不畫來源標籤、抓取時間、情緒符號），再接本步寫的六領域「知識庫今天學到什麼」。技術更新／付費方案／媒體報導／技術熱度討論這些區塊不上站。**本步不用抄聚焦與重點話題進 `daily/`**——`build_web.py` 直接沿用 `news/` 的解析結果，`daily/TARGET_DATE.md` 只寫六領域。使用者裁決見沿革檔 2026-09-12 乙-2。
...
3. **寫 `daily/TARGET_DATE.md`**，格式依上表契約字串：
   ...
   - **事實句的主詞必須是世界上的東西 `[加入: 2026-09-12 乙-2]`**（一次發布、一份量測、一篇實測、一個數字、一個人說了什麼），**不得是本庫的頁面、表格、目錄**。自檢法：把 wikilink 拿掉，那句還說得出 Claude／Anthropic／生態發生了什麼嗎？說不出就不寫。總結句同樣。知識庫自身的整理——拆頁、併頁、換版面、汰除舊列、新增主題頁、表格重排——一律不算學到，除非那次整理本身改變了某個結論（那就寫結論，不寫整理）。首份樣本六條裡三條主詞是頁面，使用者原話「每個領域都有一樣問題」。`scripts/check_reader_digest.py` 以整理語黑名單機械看守
