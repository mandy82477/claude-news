 ...
 **🧰 行的紀律**：連頁＋引症狀句，不連錨（錨點會腐化）；只引決策表**首選**與必要分界，不複製工具細節（事實的家在 tools 頁）；引用的首選被更換時本行必須同步——症狀句對帳由 `scripts/check_tools_page.py` 機械驗（引錯句測試會紅），症狀列被移除時本行降為缺口態。**缺口態格式**：`—（決策表暫無對應列｜候選症狀：X）`。
