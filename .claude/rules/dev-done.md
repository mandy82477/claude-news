...
1. **測試綠**：`python scripts/run_tests.py` 通過。`.claude/hooks/check_tests_on_stop.py`（Stop hook）會在 `src/`、`scripts/`、`.claude/hooks/` 有未 commit 改動且測試未綠時擋收工；規則檔另由 `check_rules_on_stop.py` 看守。
