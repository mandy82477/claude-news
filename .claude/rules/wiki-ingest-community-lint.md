...
**通用契約住 `.claude/rules/wiki-ingest-format.md`「時段蒸餾與封存（全站通用）」**——對象判準、≥ 3 個月門檻、三條例外、≤ 15 行時段總結、archive 子頁規格、引用回掃、每頁至多 2 個時段、只動「最後更新」，全部照該節，本節不重抄（2026-09-04 全站泛化：條文複製到六份規則檔必然漂移）。以下只列社群兩頁的特有部分。
