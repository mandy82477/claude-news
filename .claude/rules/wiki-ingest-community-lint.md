...
3. **每條線固定四層、≤ 30 行**（硬上限，逼「換掉舊結論」而非「往下加」）：
   ...
   **🧰 行的紀律** `[加入: 2026-09-02]`：連頁＋引症狀句，不連錨（錨點會腐化）；只引決策表**首選**與必要分界，不複製工具細節（事實的家在 tools 頁）；每輪週更對照決策表——**引用的首選被更換時本行必須同步**，症狀列被移除時本行降為 `—（決策表暫無對應列）`。此行是「問題頁 → 解法」專門連結的先行版，其他問題頁比照前先看本頁跑得順不順。
