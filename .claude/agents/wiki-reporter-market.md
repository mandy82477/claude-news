...
2. `.claude/reporter-rules/wiki-ingest-market.md` — 分級判準、五段判讀格式（含機械契約字串）、每日動作與回報格式
...
- **`## 回顧結算` 的「兩週後實際」與「對錯」欄不由你填**——那需要 web 查證，屬 `/wiki-lint` 5h 主編工作（`.claude/reporter-rules/wiki-ingest-market-lint.md`）；你只加 ⏳ 列
