...
2. `.claude/reporter-rules/models/daily.md` — 負責頁面清單與模型更新規則
3. 頁面表格契約 → `.claude/reporter-rules/models/pages.md`——**一頁一節，只讀本次要動的頁的那幾節**（先 Grep `^## ` 取節名與行號，再 Read 該範圍；不動的頁不讀，週更整線重寫時才整檔讀）
