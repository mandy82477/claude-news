...
2. `.claude/reporter-rules/wiki-ingest-community.md` — 負責頁面清單、討論升格與保留規則
...
**收錄門檻：** 採用訊號（達對照表 `.claude/reporter-rules/wiki-reporter-shared.md` 中門檻／跨來源 source_count ≥ 2／實際回饋）+ 複現性（repo/可跑指令/數字）；無 repo/demo/連結的純主張、行銷稿不收。
