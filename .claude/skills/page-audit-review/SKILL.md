...
3. **拆頁與開頁的單位是問題，不是行數。** 開新頁要三個訊號同時亮：有人問而全站答不了（冷讀者連兩波沒拿到或 Query 史問過）、答案會一直變（有自己的時序；靜態對照是母頁的一節）、同一件事已住在兩三頁各記一份。一頁要答的題分成兩群（不同讀者或不同時序）就拆成**子頁**：母頁留一句指路、子頁掛 `**上層：**`、index 用投影不佔列；候選按讀者類型列，不按節列。逾 600 行是鬧鐘：先蒸餾，仍逾則回頭看考題集是不是太多題（分群→子頁，母頁留索引＋重要結論）；缺入口→index 加路由列；兩頁合起來能答→index 一列寫「A 看這、B 看那」；死案→歸檔。這些都不是開新故事頁。
