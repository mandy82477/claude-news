...
3. **拆頁與開頁的單位是問題；行數是訊號，不是禁區。** 開新頁要三個訊號同時亮：有人問而全站答不了（冷讀者連兩波沒拿到或 Query 史問過）、答案會一直變（有自己的時序；靜態對照是母頁的一節）、同一件事已住在兩三頁各記一份。一頁要答的題分成兩群（不同讀者或不同時序）就拆成**子頁**：母頁留一句指路、子頁掛 `**上層：**`、index 用投影不佔列；候選按讀者類型列，不按節列。逾 600 行是鬧鐘：先蒸餾，仍逾則回頭看考題集是不是太多題（分群→子頁，母頁留索引＋重要結論）；蒸餾後仍逾 600 行且考題分得出群，行數本身可列為拆子頁的加分訊號，不必等三訊號同時亮（使用者 2026-09-23）；缺入口→index 加路由列；兩頁合起來能答→index 一列寫「A 看這、B 看那」；死案→歸檔。這些都不是開新故事頁。
4. **使命句不能太硬**：資料撐不起的裁決句會製造假結論；不虛報優先於行動導向。行動導向仍要有：每則有「接下來看什麼」「你的選項」，列選項不下指令。
5. **輸出要短**：結論五行內講完，細節下沉；表格只在有得比時用（存在條件見 `.claude/reporter-rules/page-templates.md`）。
