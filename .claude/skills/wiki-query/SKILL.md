...
| 概念、選型、「哪頁在講 X」 | 2（`python scripts/wiki_search.py "<問題原句>"` 全文排序 → 對照 `wiki/index.md` 補挑 → **候選頁全開**，讀頁頂 callout 與命中段） |
...
第 2 路的候選頁**全部要開**，不得憑 index 摘要或關鍵字字面再篩一次——摘要不含正文用詞，篩了就是在查證前先漏掉答案。候選少於 3 頁、有 ⚠ 低覆蓋、或零命中（exit 1）→ 加 `--expand` 再跑一次（以命中頁為種子沿 wikilink 圖擴散一跳，撈「相關但沒共用任何詞」的頁，結果標「圖擴散」＋種子名）；仍零命中而你判斷 wiki 該有 → 把使用者的用詞加進 `data/search_aliases.json` 對應叢集再查一次，並在回報說明。
