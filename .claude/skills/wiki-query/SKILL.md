...
| 概念、選型、「哪頁在講 X」 | 2（先把問句改寫成幾種說法 → `python scripts/wiki_search.py "<問題原句>" "<改寫 1>" "<改寫 2>" …` 全文排序 → 對照 `wiki/index.md` 補挑 → **候選頁全開**，讀頁頂 callout 與命中段） |
...
第 2 路的檢索只比字面，語意由你補：跑之前先改寫問句，原句永遠放第一個參數，改寫怎麼寫見 `.claude/skills/wiki-query/references/contract.md`「查詢改寫」。回報時列出實際用了哪幾種說法。

第 2 路的候選頁**全部要開**，不得憑 index 摘要或關鍵字字面再篩一次——摘要不含正文用詞，篩了就是在查證前先漏掉答案。候選少於 3 頁、有 ⚠ 低覆蓋、或零命中（exit 1）→ 加 `--expand` 再跑一次（以命中頁為種子沿 wikilink 圖擴散一跳，撈「相關但沒共用任何詞」的頁，結果標「圖擴散」＋種子名）；仍零命中而你判斷 wiki 該有 → 讀 `wiki/index.md` 學這個主題在庫內的用詞，換一批改寫再查一次，並在回報說明。
