...
**3. 補跑不套用跨日去重快取**（`main.py` 明文：backfill 不碰 cache，否則會拿今日的快取去誤刪過去的項目）。因此補出來的日報**可能與前後日的日報有重複條目**，屬預期行為，wiki ingest 端由 `.claude/skills/wiki-ingest/SKILL.md` 的「確認最近是否已處理過同一份日報」把關。
