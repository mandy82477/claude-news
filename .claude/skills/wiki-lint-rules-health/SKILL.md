...
⚠️ **日報有三種歸因格式，必須同時計** `[改版: 2026-09-04]`：`（ref: url）` 行內式為 2026-07-24 以前；`[N]` 註腳＋檔尾清單為 2026-07-25～2026-09-03；`（[來源名](url)）` 行內連結為 2026-09-04 起（格式定義見 `.claude/skills/news-digest/selection.md`「📌 今日聚焦」）。**只計其中一兩種，會把新格式的日子讀成 0 而誤報覆蓋率暴跌**。此為「回歸偵測器自己壞掉」的前例，改日報格式時必須回頭同步本節。
