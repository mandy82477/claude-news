...
| 行動清單小標 `### 本週要動的事`（第二節最後一個 h3）；無事時內文 `本週沒有需要動的事` | `build_web.py` 討論子段切分 → `discussion.actions`、`app.js` `.weekly-actions`、`check_weekly_ledger.py` ACTIONS_HEADING_RE／ACTIONS_NONE（W39 起硬擋） | 清單不上站；檢查器判為缺漏 |
