...
- 品質指標（6g）：
  ...
  - 外部死鏈（每週）：共 N 條疑似死鏈，已標註 M 條（報告過期則寫「⚠️ link_health.json 過期，本輪不據以標註」）
  - 趨勢判讀：（持平／惡化中／已回升，一句話）
...
- 待使用者裁示：（逐項列出，**每項標「⏳ 已擱置 N 週」**——N 由 Grep 前幾次 lint 紀錄同一事項推得；無則寫「無」）
```

> 擱置週數是 `/weekly` 步驟 0 呈報時的排序依據（使用者不讀 `wiki/log.md`）。（沿革檔 2026-08-28 E）
