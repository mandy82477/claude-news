...
1. **盤點＋排序**：先跑**完整報告**再跑佇列——
 ...
 - **Lane B**：需 WebFetch 官方一手來源查證，受 egress 與成本限制
   ...
   > 為何仍分流（額度已於 2026-09-20 取消，分流保留）：兩條 Lane 的成本結構不同，執行者需要知道哪些要花 web 預算、哪些不用。原本分流是為了防「便宜那種佔滿額度」（沿革檔 2026-08-29 A），現在沒有額度可佔，但成本標示的價值不變。

 **🎯 本輪目標那一行必須讀，並抄進本步回報**：它印出「清到 0（待清 N 筆；近 7 天新增 M 筆）」。**只看「總逾期數」看不出流量**，「📈 趨勢」一行一併抄進回報。

 **清零制（2026-09-20 使用者裁示）**：本步目標是把逾期清到 **0**，沒有每輪額度。**按頁面所有權拆給各類別記者並行**（六記者各清自己領域的頁），不是主編一個人查完——舊制額度（Lane A 10／B 8）的前提正是「主編一個人做」，於是排空要 92 天而複查日只給 30 天，每一筆都必然在輪到它之前先逾期，「逾期數」淪為佇列長度的別名。實證：2026-09-20 六記者並行一輪清掉 109 筆。瓶頸從來不是總量，是只有一個人做。
...
- **連續 2 輪同一頁仍在清單且處置皆為 c** → 該頁型態與判準不合，**不要調全域門檻**（`STALE_DAYS`／`INBOUND_HIGH` 一動，全庫的真停滯也跟著被放過）。改在該頁宣告一行豁免：

  ```
  **新鮮度豁免：** <理由：為什麼這頁的維護規則不動「最後新聞更新」>
  ```

  `gen_wiki_frontmatter.py` 讀到就把該頁的 signal 改成 `🗓️ 非新聞驅動`、理由寫進 `staleness_exempt` 欄。**豁免不等於隱藏**：它有自己的一格、分布行看得到，只是本步的清單撈不到。理由留白等於沒宣告（regex 要求非空白），避免無聲豁免。

  > 判斷式：這頁停滯是**事實**（該寫沒寫），還是**判準不適用**（它本來就不吃新聞）？前者照上面二選一處置，後者才宣告豁免。
...
對 `wiki/entities/fable-5.md` 與 `wiki/entities/opus-5.md` 的結論表各跑一次（判準見 `.claude/reporter-rules/models/pages.md`「模型頁兩代對照表」與「entities/opus-5 的兩張表」）：① 兩頁 `## 你現在拿到的是什麼` 表上「資料截至」距今逾 60 天 → WebFetch 官方模型總覽頁重查七列並更新查證日；② fable-5 `## 護欄會怎麼改寫你的請求` 四類是否仍為官方公布的四類；③ opus-5 `## 這些數字是誰量的` 的官方基準是否仍為官方在引用的那幾項，社群側是否首次出現帶測試方法與數字的獨立複測（有則同批改寫導言那句承諾）；④ 兩頁 `## 熱度與試用價值` 對 feature-radar 全覽表對應世代那一列，不一致以 radar 為準覆寫；⑤ 所有 `status_main: active` 且非 Legacy 的模型頁 `## 跟它怎麼說話`（判準見同檔「各現行模型頁共用」節）：缺節或留著「待主編查證」行 → WebFetch 官方 migration guide 對應模型的 prompting 節補寫；查證日距今逾 90 天 → 重查並更新查證日。

**回報格式：**
```
模型頁世代表（5l）：fable-5 資料截至 YYYY-MM-DD（重查 N 列）／護欄四類 一致 or 變動 M 項；opus-5 資料截至 YYYY-MM-DD（重查 N 列）／官方基準 一致 or 變動 K 項／社群獨立複測 無 or 已出現；熱度表 一致 or 已覆寫；跟它怎麼說話 N 頁齊全／補寫 M 頁／重查 K 頁
```
