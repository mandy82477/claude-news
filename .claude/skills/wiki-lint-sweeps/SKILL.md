...
由 `.claude/skills/wiki-lint/SKILL.md` 在 A 段（記者收報）之後帶起。**這十三步的觸發邊只有這裡**——它們吃的是外部榜單、官方文件、GitHub issue、圖譜訊號或帳本，類別路由接不到，記者也沒有 web 工具。
...
- 雲端執行 5b／5c／5m 前先跑 egress 探測（**不得未探測就跳過**）；5h 不需探測。判斷表見 `.claude/skills/wiki-lint-sweeps/references/sweeps.md`「雲端 egress 探測」節。
