...
**本 routine 一天排六班（12/14/16/18/20/22 UTC），你是其中一班。**

常態日第一班就把日報做完，其餘五班撞 `Step 0b：冪等閘` 直接結束。上游抓料若被 GitHub 延遲（實測最多 +11.2 小時），資料到齊的那一班接手，日報晚幾小時上站但不會消失。**沒有一班需要「等」**——這取代了 2026-08-29 短暫用過的「單班 + 11.6 小時緩衝」，那個緩衝在常態日是純粹空等，卻讓送達由台北 21:00 變成隔日 06:00。

> **空跑的班次完全靜默**（見 `_shared.md`「無人值守原則」）：撞冪等閘、或資料尚未到齊而你不是最後一班（22:00 UTC）→ 不寫標記、不寫心跳、不 commit，直接結束並回報一行。只有最後一班仍等不到資料，才寫 ABORTED 心跳並 commit push。
