...
| `5b. 跨家任務榜單週更（主編派工）` | **雲端一律跳過**（需抓外部榜單網站，egress 封鎖，實測 `www.swebench.com` 回 `EGRESS_BLOCKED`）→ 寫入待辦「留待本機執行」 |
| `5c. 逾期待查證清算（主編親查）` | **雲端一律跳過**（需查證官方說明中心等外部網域，egress 封鎖）→ 寫入待辦「留待本機執行」 |
