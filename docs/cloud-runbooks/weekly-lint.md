...
**✅ 外部死鏈檢查已移交 GitHub Actions，雲端照常執行 `[改版: 2026-08-20]`**：不再自己連網——改讀 `data/link_health.json`（由 `.github/workflows/weekly-linkcheck.yml` 週五 20:00 UTC 產出並 commit 回 repo），因此雲端可完整執行指標三、不需跳過、不需留待辦。**唯一要守的是新鮮度防線**：`checked_at` 距今 > 10 天視為報告過期，本輪不據以標註頁面，改回報「⚠️ link_health.json 過期」——過期報告比沒有報告更危險，它看起來像剛檢查過。（原設計在雲端跑 `check_links.py` 會把全部連結誤判為死鏈並污染頁面，故當時標為跳過；移交 Actions 後該風險消失。）月度蒸餾的立法提案仍屬「要求確認」→ 照上表寫入待辦；採用驗證率是本地檔案統計，雲端可自主執行。
