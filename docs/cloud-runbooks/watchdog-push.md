...
時間點的理由：日更 pipeline 最後一班 22:00 UTC 開跑、約 30–60 分鐘完成，GH watchdog 01:00 UTC 覆核。01:30 UTC 時前一日產出已定案，而使用者剛起床——這是「知道了還來得及在今天處理」的最早時刻。**不要往前挪**，那會落在 pipeline 還沒跑完的時候，天天誤報。

檢查對象是**前一個 UTC 日**（即剛跑完的那一輪；routine 最後一班 22:00 UTC，本 routine 在隔日 01:30 才執行），不是執行當下的 UTC 日期、也不是台北日期。

---

## 步驟

1. 確認工作目錄為 repo 根（專案根即 repo 根）。
