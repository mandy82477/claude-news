...
| 11 | entities/opus-5（廣度優先第 1 層第 7 頁，入邊 33、162 行、逾期懸置 3） | 樞紐（低標） | 2026-09-07 | **進行中（設計者已派）**。使命句（主 session 定，健檢卡 A）「Opus 5 這個模型本身是什麼——規格、官方與第三方基準的位置、以及上線 44 天來社群到底吵出了什麼」。考題五群、拆子頁候選 0（三群家在鄰居頁、留舊副本）；opus-4-8 留（仍是 Fable 分類器 fallback，活產品行為）。代判：三筆逾期懸置依查證結案（兩筆不可查證、一筆來源不可靠）、結論表沿用 fable-5 兩代對照形狀；**裁決點交使用者：opus-4-8「Opus 5 傳聞」12 行死案歸檔（跨頁砍節，預設不動、記回訪）**；代判：opus-4-8 三處事實更正（Legacy＋退役日 2027-05-28，index 狀態「已被取代」→「active（Legacy）」）本波同批改（同維護者事實更正必回掃，比照第 10 波）；設計者交件 16,593→14,491 字元（行數 162→168，規模改用字元量）。報價：頁短，健檢卡＋冷讀者＋設計＋評審＋複驗 5 新 agent＋續用 3，估 60–90 萬 token。已知：歷史記錄 2×15 prepend 無機制、熱度表 unknown；index L54 鉤子「定價定位待彙整」；三筆逾期懸置 L152／155／158；fable-5 第 10 波已建兩代對照表（Opus 5 是官方「most workloads」預設推薦，$5／$25、退役不早於 2027-07-24） | — | — | `opus-5-2026-09-07*.md`、`wave11-cold-reader{,-recheck}-2026-09-07.md` |
