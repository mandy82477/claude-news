...
| 10 | entities/fable-5（廣度優先第 1 層第 6 頁，入邊 43、412 行、5 張表全 unknown 無機制） | 樞紐 | 2026-09-07 | **進行中（設計者已派）**。使命句（主 session 定，健檢卡 A）「Fable 這一代模型本身是什麼、護欄會怎麼改寫你送進去的請求，以及它一路走過來的事」。**考題分群結果：拆子頁候選 0**——五群裡只有護欄群的家在本頁，存取／價格、出口管制、世代身分的答案都住已定稿鄰居頁，本頁的病是舊副本不是問題太多（新判準首次套用，結論是不拆）。代判：出口管制 120 行（雙方立場表＋06 月歷史）死案歸檔→新建 `entities/fable-5-archive`（比照前波）；費馬事件移出本頁；「靜默降級」四句互斥依官方改寫並結 H-754509。報價：5 新 agent＋4 續用，估 80–120 萬 token（頁短）。已知：現況節四段不同日期並陳（09-04 費馬事件、09-01 Fable 5.1 發布、06-09 發布、07-20 狀態）；「配額與計費過渡」節名帶已過期日期；歷史記錄三段 200 行（解禁後／管制期／管制前）；出口管制雙方立場兩張 static 表；本波首次套用新拆頁判準（考題分群）與 09-07 skill | — | — | `fable-5-2026-09-07*.md`、`wave10-cold-reader{,-recheck}-2026-09-07.md` |
