...
- **2026-09-07｜拆分第三切口「證據簿」**（使用者：「行數太多也可以考慮拆」「母頁也可以變索引頁跟重要結論」「我沒有說只能拆 log」）：`wiki-ingest-format.md` 頁面拆分原則加 (c) 證據簿——逾 600 行先蒸餾、仍逾則活事件流整段下沉為子頁（不參與三題、比照 archive），母頁＝結論表＋子頁路由表、不得只剩目錄。**第一批：patterns（1,154 行）與 discussions（回訪 09-13）技術彙整下沉為證據簿子頁**——各自仍是拆頁裁決點，動手前逐頁問。
