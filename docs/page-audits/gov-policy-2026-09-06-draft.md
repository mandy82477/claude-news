...
**不可按區塊剪貼：** 2026-06 的 60 列在主頁**不連續**——`L320`（`2026-06-30～07-10`，指向中國頁的跨月列）夾在三列 07 月列（L321–323）**之上**，其餘 59 列是 L324–382。必須逐列判月份再搬。
...
`Anthropic 政府政策攻防：出口管制 2026-06-30 已解除、07-01 恢復存取；現在八條線在動，其中香港與中國大陸不在許可存取區、輸出帶隱形浮水印、高風險請求換 Opus 4.8 三條已經改到你手上的 Claude`
...
`--from 安全政策 --to 模型 --page entities/fable-5 --note "兩件事實更正，請回掃。(1) 出口管制的日期與天數：正解為 2026-06-12 商務部 BIS 管制生效 → 06-30 商務部通知已移除 → 07-01 恢復存取，封鎖 19 天（含首尾兩日；沒有『含首尾』三個字讀者自己算會是 18）。全庫實掃 9 處寫成 06-13／07-01／06-26 等舊組合，請逐處改：entities/fable-5 L48（06-13 至 07-01，歷時 18 天）、L303（6/13–7/1）、L319（管制事件歷時 18 天，2026-06-13 至 2026-07-01）、L321（節標 出口管制期（2026-06-13 至 06-30））；entities/mythos L46（歷時 18 天，2026-06-13 至 2026-07-01）、L222（節標 出口管制期（2026-06-13 至 06-26），第三個結束日）；entities/opus-4-8 L46（出口管制期間 2026-06-13 至 07-01）；entities/tom-brown L42、L67（封鎖期共 18–19 天）。天數 18 本身多處是對的，錯的是端點。L321 與 L222 是節標，改名前先跑 python scripts/wiki_graph.py explain entities/fable-5 --section 出口管制期（2026-06-13 至 06-30） 與同法對 mythos 確認無錨點邊。L303 是歷史記錄裡的敘述，只改端點不改敘述。(2) entities/fable-5 L103 寫『觸發時靜默 fallback 至 Opus 4.8』，與官方 06-30 Redeploying Claude Fable 5 原文 Users will be notified if a request to Fable 5 is blocked 牴觸，請以官方原文為準或標懸置；L310 不必改，它下一行 L313 已寫明是可見防護。另請在該頁補一句回指 topics/anthropic-government-policy 的『政府動作對你的產品做了什麼』完成雙向連結；分類器機制細節（<5% session、生物領域 85% 改善）家在 fable-5，政策後果家在政策頁，兩邊互指不複製。"`

**這筆轉知的射程比評審給的多 3 處**：評審列 6 處（fable-5 四、mythos:222、tom-brown:67），實掃另有 `entities/mythos:46`、`entities/opus-4-8:46`、`entities/tom-brown:42`。掃法：`grep -rn "18 天\|2026-06-13 至\|6/13" wiki/ --include=*.md`，排除 `log.md` 與 `pricing`／`community-tech-discussions` 中另一個「延燒 18 天」的 issue（同字串不同事實）。

## M. 給人物記者的轉知逐字（`entities/dario-amodei` 四處，走 `scripts/pending_handoffs.py open`）

`--from 安全政策 --to 人物 --page entities/dario-amodei --note "chris-olah／tom-brown 已併入 topics/anthropic-government-policy 並留轉址殼（2026-09-06）。L83／L84 兩列相關實體請改指 [[topics/anthropic-government-policy#誰在動這幾條線]]；L44／L74 的散文別名引用留原樣（講的是創辦背景，不是政策動作）。殼仍解析得到，連結不會斷，但相關實體清單會把讀者送進兩個空殼。"`
