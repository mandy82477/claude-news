...
**checkpoint 後五處改動的重核（磁碟現行檔）。** `cc28e105` 之後實作者又改了 5 處，我對受影響的三項重查：

1. **轉址殼**：`resolved（已併回）`、`**上層：** [[topics/community-tech-patterns-archive]]`、blockquote 已去、識別句改成平述一行；frontmatter 已重生成（`parent` 指 archive、`page_role: redirect`、`inbound_links: 0`）。✅
2. **allowlist 撤銷 timeline「封存」那筆**：全庫 `封存` 例外現在 **0 筆** ✅。但 timeline 仍留著一筆 `主編`／「自主編程」的例外，**那行文字已隨併頁搬進 archive、殼裡不存在了**——archive 自己已有同義例外，這筆是死條文，列回訪清理。
3. **archive `## 2026-04`** 底下直接是 `### 時序流水帳（併自原社群時序頁，2026-04-25～04-30）` 再接 `#### 2026-04-30`，多加的反向指標句已移除 ✅。
5. **「與官方缺口矩陣互見」已補回，而且是 map 判「改」的版本不是原句**：現行句寫出雙方視角與「查什麼去哪一頁」（:155），比 HEAD 前的「完整追蹤見該頁」明確，`H-c1aa57` 的我方那一半成立。✅

**我上一版對這段刪除的歸因是錯的，更正：** 我當時查到工作區、暫存區、HEAD 三處都有那段，就判成「另一個 agent 編輯中的瞬間狀態、已自行收斂」——實際上那是實作者真的誤刪、經主 session 要求補回，我查的時點正好在補回之後。**教訓歸誰要查得出來才算數**：31 個未提交檔、多 agent 共寫的工作區裡，單次 `git diff` 快照既不足以斷定一段被刪掉，也不足以斷定沒人刪過；要斷定歸屬得問改的人，不能從 git 狀態反推。

重跑全閘：`check_hierarchy` ✅（7 個母頁）、讀者語言無新增命中、懸置 142 筆對帳 ✅、`run_tests.py` exit 0、`build_web` 斷鏈 13／錨點 0。

其餘無 🔴。**回訪五項：** `table_census` 成長型仍判 `unknown`（加了日期欄未如提案預期翻成 `accumulate`）／index L27「我卡住了」誤導列仍在首屏前段，是 A-8 的刻意取捨，複驗 Q1 因此仍判半拿到／讀者語言 allowlist 兩筆「覆寫」可改寫掉、少兩筆例外／**timeline 那筆「自主編程」例外已成死條文，該刪**／工具數不一致要進 2026-09-13 回訪清單。
