# 逐字稿：`wiki/topics/anthropic-agent-stack.md`（第 13 波，2026-09-12）

設計者交件三之三。分「進頁面」「進規則檔」兩欄。frontmatter（L1–24）由 `scripts/gen_wiki_frontmatter.py` 生成，本稿不寫。
新版正文 234 行（含 frontmatter 共 258 行；原 116 行）。兩支閘實測結果見 `-proposal.md` 末節。

---

# 一、進頁面（A）：`wiki/topics/anthropic-agent-stack.md` 新版全文

以下 fence 內即為 frontmatter 之後的全部正文，可直接整段取代 L25–116。
...
> **最近一次積木層級變動**（2026-09-10）：dynamic workflows 開放給全部付費方案（Pro 需在 `/config` 開啟），編排從「Claude 逐回合決定」換成一支可存檔重跑的 script。
...
- **還做不到什麼**：管的是單一 session 內跑得完的事，沒有跨 session 記憶。回合結束時若還有 subagent 或背景 shell 在跑，那一回合的檢查會被跳過，等下一回合再判。
...
- **還做不到什麼**：Explore 與 Plan 唯讀、一次性不能追問，且會跳過 CLAUDE.md 與上層 session 的 git status（官方明說只有這兩個省略）；模型繼承主對話、在 Claude API 上以 Opus 為上限，要來回追問改用 `general-purpose`。**subagent 裡開不了 workflow**。
...
```
audit every route handler under src/routes/ for missing authentication checks, and adversarially verify each finding
run `npx tsc --noEmit` and keep fixing until the type check passes or two rounds make no progress
migrate every component from JS to TS, each file in its own isolated copy
review every file changed in this PR then merge into one ranked summary
research how three competitors handle rate limiting in parallel then compare
find flaky tests: run the suite repeatedly, stop once two rounds find nothing new
```

  怎麼叫它：prompt 帶 `ultracode` 或直說 use a workflow；`/effort ultracode` 讓 Claude 自己決定每個實質任務要不要開；內建範例 `/deep-research`。

- **一支最小 script**：存成 `.claude/workflows/audit-routes.js`（或個人目錄），用 `/audit-routes` 叫它跑、`/workflows` 看進度。形狀取自官方 `audit-routes` 範例：

```js
export const meta = {
  name: "audit-routes",
  description: "掃每個 route handler 的驗證缺口，再各自對抗式覆核一次",
};

export default async function ({ agent, parallel, pipeline }) {
  const list = await agent("列出 src/routes/ 下所有 route handler 檔名，一行一個");
  const files = list.split("\n").filter(Boolean);
  const findings = await parallel(files.map((f) => agent(`檢查 ${f} 有沒有漏掉的驗證`)));
  return pipeline(findings, (f) => agent(`對抗式覆核這則發現，說得出哪裡站不住：${f}`));
}
```

  四原語：`agent()` 派一個、`parallel()` 齊發等全收、`pipeline()` 逐項流水、`phase()` 分階段。同一 session 內重啟時，已完成且輸入沒變的 agent 直接回快取結果，只重跑失敗者與其後手（[官方 workflows 文件](https://code.claude.com/docs/en/workflows)，2026-09-12 查證）。
...
```
Review PR #142 with three reviewers: one on security, one on performance, one on test coverage
Spin up five teammates, each holding a different hypothesis, and have them argue it out like a scientific debate
```
...
- **還做不到什麼**：只傳純文字，不傳對話歷史與檔案，訊息也不帶依賴語意——要排誰先誰後得靠 workflow。當初那張需求工單（[issue #24798](https://github.com/anthropics/claude-code/issues/24798)，已關閉、84 則留言）要的三件事裡，官方對上的是跨 session 傳訊與閒置通知，「共用專案暫存區」至今沒有對應品；協定級的 agent 互通（A2A）仍是未回應的功能請求（[issue #28300](https://github.com/anthropics/claude-code/issues/28300)）。
...
「subagent 加 workflows」這個「加」字的答案在這裡。官方文件說得出口的只有這四條（2026-09-12 查證），其餘組合官方沒寫，本頁不猜：
...
# 二、進頁面（B）：子頁 `wiki/entities/managed-agents.md` 兩處（Q4 卡點，計費歸家）

母頁刪掉計費算式後，子頁必須接住它，否則兩頁都指不到金額。**只動這兩處，子頁其他任何行不碰。**

**B-1　取代子頁 L59 那句指路（刪掉「計費算式與官方算例……見上層」）：**

```
> 跨功能的熱度對比見 [[feature-radar]]；跟其他官方 agent 形態怎麼挑、八塊積木各自為什麼出，見上層 [[topics/anthropic-agent-stack]]。
```

**B-2　在子頁 `## 熱度與試用價值` 之後、`## 接下來看什麼` 之前新增一節**（內容為母頁 L68–L69 兩條逐字搬入，一字未改）：
...
**C-1　`wiki/feature-radar.md` L227 只改狀態欄**（第 5 欄，`Research Preview` → 現況；名稱欄的退款爭議、熱度欄、試用價值欄一字不動）：
...
**C-2　`wiki/index.md` L30 路由列微調措辭**（不新增列；括號補 agent teams 與 agent view，讓 Q3 讀者對得上號；連結目標與引號節名不變）：
...
# 四、進規則檔：`.claude/reporter-rules/wiki-ingest-features.md`（三處，逐字）

**規則-1　取代「agent 相關功能：多標三件事」節中「判定為 agent 相關時……」那一段與其下的三點清單**（節名、觸發句、末尾判斷式皆不動）：
...
**規則-2　「Managed Agents 零件狀態表的回訪」節，取代「母頁 [[topics/anthropic-agent-stack]] 的 `## 你該用哪個` 表……」那一句**（必須維持單行，`table_census.py` `_mechanism()` L50 是逐行比對）：

```
母頁 [[topics/anthropic-agent-stack]] 的 `## 附錄：五種形態速查表`「現在拿得到嗎」欄與本表同源（2026-09-12 起選型表降為該頁附錄），一併**覆寫**、不得留舊值；同一次順檢母頁八張積木卡第三欄（還做不到什麼）的狀態詞與「子頁怎麼分」表的最後動態欄。
```

**規則-3　「負責頁面」表 `wiki/topics/anthropic-agent-stack.md` 那列的觸發條件整格取代：**
...
> 沿革檔 `docs/rules-changelog/wiki-ingest-features.md` 補 2026-09-12 段（為什麼把落點從「架構層級」軸換成「一積木一卡」軸：16 格盤點 11 格「無」的成因是軸選錯，不是漏寫）；條文只留判準，依 `.claude/rules/claude-md-edit.md`。
