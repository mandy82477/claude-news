...
| L33–37 | 頂部 callout（2 則，2026-09-05） | **留，一字不動** | 健檢卡判合格 |
| L40–50 | `## 摘要` 5 段 | **改**（整節替換） | draft A-1。首句改成 delta；L44 三頁分工句拆成「哪一頁答什麼」；trends 入口補「哪個方向在加溫」 |
| L48 | 官方 context engineering 指引段（42 天，寫在摘要像新聞） | **下沉，不是刪除** | 移進 `### 2026-07` 分組，改成 `#### 官方 context engineering 新規則：移除逾 80% 系統提示詞（2026-07-26）` 一則節點，四個 wikilink 一併帶走 |
| L52 | `## 模式概覽` 節名 | 留（`wiki_graph explain --section "模式概覽"` 回錨點邊無，改名零風險但無必要） | — |
| L53–55 | 表頭 4 欄 | **改為 5 欄**（＋「最後動態」） | draft A-2 |
| L56–59 | ✅ 4 列 | **留＋補日期與月份錨點**，順序改為 Skills 設計／Multi-agent／CLAUDE.md／Hooks（同級依最後動態新者在上） | 09-05／09-02／08-04／08-02 |
...
| L67–70、L72–76、L79–80 | ⏳ 12 列 | **留＋補日期與錨點**，依最後動態降序排 | 08-27→07-12 |
| **L71** | ⏳ 跨環境 Agent 記憶（本頁零節點，唯一出處在 archive 2026-05-12） | **併入 L62** | 代表技巧 Core Memory Packet 與 L62 完全重疊——四次提案的同一組 |
| **L77** | ⏳ Agent 記憶保護（最後動態 2026-06-30，68 天） | **移出表，降表下一行** | draft A-2 表下「兩類已超過 60 天」，附 `[[topics/community-tech-patterns-archive#2026-06]]` |
| **L78** | ⏳ 跨 Repo 依賴可視化（最後動態 2026-06-30，68 天） | **移出表，降表下一行** | 同上 |
...
| L110 | `### 誰負責拆分` 節名 | 留 | — |
| L111–125 | 拆分來源表 3 欄 × 4 列 | **改為 4 欄 × 5 列**：新增「官方怎麼說（2026-09-06 查證）」欄；新增「模型自動委派」列；「peer 之間協商湧現」列**更正**為「lead 拆分後隊友自領」 | verified 第一節逐字（Claude automatically delegates…）、第二節逐字（The lead breaks work into tasks…；自領 claim 用 file locking） |
| L127 | 「核心經驗（文獻）」四條 | **留，加日期**「文獻怎麼說（截至 2026-07-22）」並壓成一段 | draft A-4 |
...
| L139 | `### 缺口追蹤` 節名 | 留 | — |
| L140 | 導言（含「狀態判定為盤點結論（推論）」） | **改**：加「資料截至 2026-09-06（官方文件查證）」、狀態三值說明；「盤點結論（推論）」改寫成讀者語言 | 冷讀者外洩第 7 條 |
| L141–152 | 缺口表 7 列，狀態 ✅／❌ | **改**：狀態符號 → 文字三值（已補／部分補上／未補），解 ✅ 撞名 | draft A-5 |
| **L148** | 「分模型 ❌ 未補」 | **翻為「已補（2026-09-06 查證）」** | verified：subagent 定義有 `model` 欄、呼叫時可另行指定、`CLAUDE_CODE_SUBAGENT_MODEL`、teams 文件建議隊友用 Sonnet。**這是冷讀者抓到的頁內自相矛盾**（L148 vs L60 官方基準 46%／96%） |
| L142／L143 | 通訊原語、脈絡交接 ✅ | **留為「已補」＋補查證日與條件** | verified：cross-session v2.1.224+（Windows 2.1.234+）、fork 繼承整段對話 |
| L146 | 動態粒度 ❌ | **留為「未補」＋補查證日** | verified 未見動態粒度，誠實寫「截至查證日未見」 |
| L149–151 | 共享計畫、協調衝突、信任驗證 ❌ | **留為「未補」**（無查證日，因無官方文件可查） | — |
...
| L676／L713／L724／L734／L752 | 5 筆完整懸置標記裸露在條列 | **改為短標記 ⟨Q-01⟩～⟨Q-05⟩**，完整式下沉該月份組末「懸置細節」 | draft A-7 |
| **L755** | 第 6 筆標記，與 L752 同節點同事實（探針同為 ip_reminder、JSONL） | **刪除（去重）** | 事實不消失——L752 的 ⟨Q-05⟩ 是它的家。刪後全庫 141→140，**必須同批 `--rebuild-count --reason`** |
...
| **L1088** | 「70+ 款工具持續追蹤」 | **砍數字**，改寫成 wikilink 指 tools 頁 | 三個數字互不一致；**本頁不是工具數的家**。tools 與 index 的不一致走轉知帳本 |
| **L1089** | 「官方量化 15 倍 token」（孤兒數字） | **改寫＋登記懸置 ⟨Q-06⟩** | 庫內唯一出處是停滯 107 天的 `community-tech-timeline` L109，無一手。不刪不虛報 |
| **L1092** | 「6/15 計費政策後爆發」（政策已 2026-06-16 暫停） | **改寫** | 「2026-06 計費切割風波（該政策已於 2026-06-16 暫停）之後」 |
| L1087／L1090／L1091 | 其餘三條結論 | **改寫**：各補一個月份錨點與「接下來看什麼」 | draft A-3 |
...
## 整頁去向（`community-tech-timeline` 併入 `community-tech-patterns-archive`）

**寫成裁決點交使用者，設計者不自行定案。預設不併。** 該頁入邊 3、`page_role: root`、`status: monitoring`、佔 `wiki/index.md` L109 一列，自述 2026-05-22 起凍結。

| 該頁的節 | 裁決 A：併入 archive | 裁決 B（預設）：不併 |
|---|---|---|
| 標頭＋callout＋摘要 | 改成 redirect 殼：領域 🌐、`**上層：** [[topics/community-tech-patterns]]`、狀態 `resolved（封存頁）`、識別字「已併回 [[topics/community-tech-patterns-archive]]」、`page_role=redirect`、index L109 刪列 | 標頭狀態 `monitoring` → `resolved（凍結頁）`；callout 改成一句「本頁凍結於 2026-05-22，之後的社群做法見 [[topics/community-tech-patterns]]」 |
| 時序條目（2026-04-25～05-22） | 原文一字不刪 append 進 archive 的 `## 2026-05`／新開 `## 2026-04` 分組 | 原地不動 |
| L109「15 倍 token」轉述 | 隨條目搬進 archive；本頁 ⟨Q-06⟩ 的指路改指 archive | 原地不動 |
| index L109 那一列 | 刪列（redirect 不入 index，`gen_wiki_frontmatter.py` 已排除） | 留列，描述改成「已凍結於 2026-05-22 的社群時序封存」 |
| 本頁 L1106 相關實體那一條 | 改指 `[[topics/community-tech-patterns-archive]]` | 描述改成「2026-04-25～05-22 的凍結時序」 |
| 三個入邊 | 本頁 L1106、archive、index——全部改指 archive 後可安全轉殼 | 不動 |

**併的理由（設計者的判斷，供裁決）：** 同一批 2026-05 的料被封存兩次（timeline 整頁 vs `patterns-archive` 的 `## 2026-05`），而兩個封存頁待遇相反——合規的 archive 不佔 index 列，凍結的 timeline 反而佔著。**不併的代價：** index 上一直有一列指向凍結 107 天的頁。**併的代價：** 要動 index 與 frontmatter 投影，且 archive 會多出一個現在沒有的 2026-04 分組。
