# 第 14 波設計者逐字稿：topics/official-community-gap（第二輪）

兩大節：**A 進頁面**（可直接貼上）、**B 進規則檔**。行號＝對象頁檔案原始行號（含 frontmatter）。
官方事實一律取自 `docs/page-audits/ocg-2026-09-19-verified.md`（含第二輪新增的「六、補查」）的逐字引文。
**分岔標示**：本檔正文照**保守預設**（裁決點 1 未回覆）寫 14 列；`〔併〕` 標記處為使用者裁「對照矩陣併入」時的 16 列版逐字。

---

# A. 進頁面

## A-1 標頭（取代 L30–31）

```markdown
**最後更新：** <實作當天>
**最後新聞更新：** <實作當天>
```

> 全頁只有 callout 一個日期口徑；摘要不再帶第二個日期（原 L39、L43 的兩個日期是第二、第三個「現在」）。frontmatter 由 `scripts/gen_wiki_frontmatter.py` 產，不手填。

## A-2 callout（取代 L33–35）

```markdown
> **AGENTS.md 已補**（<實作當天>）
> Claude Code v2.1.277（09-18）起原生讀 `AGENTS.md`，不必再寫 `@AGENTS.md` import 或做 symlink。三個邊界：有 `CLAUDE.md` 時預設仍只讀 `CLAUDE.md`；要兩者並讀得在 `/config` 把「Project instructions」改成 `claude-md-and-agents-md`；Bedrock／Vertex／Foundry 或關掉遙測的 session 讀不到。
> 同批更正：2026-08-17 官方一次關掉本頁引用的五個 issue（#6235、#24798、#47023、#24316、#29006），本頁原先把它們當成還在累積的社群壓力。
```

## A-3 摘要（取代 L39–43，L41 分工句併入第二段）

```markdown
**2026-09-18 官方補上了本頁掛最久的那一個**：Claude Code 自 v2.1.277 起原生讀 `AGENTS.md`，這一列從「全站讚數最高的未解缺口」變成「已補，但有三個邊界」。

本頁只答一件事：社群喊的痛，官方補了哪幾個、哪幾個還沒補、為什麼沒補，沒補的你現在有什麼選項。**要裝哪個社群工具，答案不在本頁**——看 [[topics/community-tech-tools]]「我卡在這裡」；官方積木各自怎麼用、怎麼疊看 [[topics/anthropic-agent-stack]]；學術文獻主張與 Claude Code 現況的落差是另一個視角，見 [[topics/community-tech-patterns#缺口追蹤：文獻主張 × Claude Code 現況]]。

13 個痛點裡，2 個官方目前沒有任何對應、7 個補了一半、4 個已經能直接用。
```

> **〔併〕** 末句改：`15 個痛點裡，4 個官方目前沒有任何對應、7 個補了一半、4 個已經能直接用。`

## A-4 新節「官方補了沒」（取代 L47–67 的節名、引言與表格）

節名：`## 官方補了沒`（實作單第 0 步先與 `features/pages.md` 同批處理，見 proposal §8）

表上引言（取代 L49–50）：

```markdown
> ✅ 官方有正式功能、能直接用｜🧪 官方有東西但受限（測試中、須申請、只給企業端）｜❌ 官方目前無對應
> 「核對日」＝這一列最後一次對過官方文件或 CHANGELOG 的日期；`—` 代表還沒對過，要拿它做決定時建議自己再查一次官方文件。
> 每個 issue 現在多少人在吵、官方回了什麼，見 [[entities/claude-code]]。
```

表格（**保守預設 13 列**，❌ 在最上、✅ 在最下）。原「Slack 內 AI 隊友」列依退場條款本波移出，見 A-4 末段與 A-9：

```markdown
| 你痛的是什麼 | 官方補了沒 | 官方給的是什麼 | 還缺什麼、你現在能怎麼辦 | 核對日 |
|---|---|---|---|---|
| 想在一個地方操作好幾家 agent | ❌ | 無——官方只管 Claude Code 自己的 session，截至 2026-09-19 官方文件未見 | 只有社群工具，08-24 起一個月冒出 10 款 ⟨G-11⟩ | 2026-09-19 |
| Agent 跟 agent 做生意、吵架怎麼判 | ❌ | 無——截至 2026-09-19 官方文件未見 | 全新類別，只有一個社群方案，而它 5,855★ 卻只有 9 位 watcher、4 筆 commit ⟨G-12⟩ | 2026-09-19 |
| 新開一個 session 它就忘光 | 🧪 | Claude Code 的 auto memory（依你的更正與偏好自己寫筆記，`/memory` 管理，子代理也各有一份） | 跨工具、跨模型、團隊共享仍要靠社群工具；要自己接外部記憶層，四個 session／compact hook 都能用 ⟨G-05⟩ | 2026-09-19 |
| 額度快用完、錢花到哪看不看得到 | 🧪 | `/usage` 拆到 skills／subagents／plugins／各 MCP server 的百分比與每個 loop 的 token、狀態列 `rate_limits`、VS Code 70% 警示橫幅 | 沒有逐次請求的金額，數字是本機估算、不含其他裝置與 claude.ai；告警門檻似乎不能自訂 ⟨G-07⟩ | 2026-09-19 |
| 想自己決定哪段用哪個模型、不被鎖住 | 🧪 | 個人端只有 `opusplan`（規劃用 Opus、執行換 Sonnet）；org default model（v2.1.196）與 `enforceAvailableModels` 是企業管理端 | 依成本或任務動態路由，截至 2026-09-19 官方文件未見；換設定差多少錢見 [[topics/model-comparison]] ⟨G-13⟩ | 2026-09-19 |
| 一堆 agent 在跑，看不到誰卡住 | 🧪 | Agent view（研究預覽）：一列顯示它在跑、在等你、還是做完了；一般 session 提示列會標 `← 2 agents` | 是狀態列表不是即時地圖；誰在等誰、依賴怎麼串仍得自己拼 ⟨G-09⟩ | 2026-09-19 |
| Agent 之間直接傳訊 | 🧪 | 跨 session 訊息 `ListAgents`＋`SendMessage`（v2.1.224 起，原生 Windows v2.1.234），可達他機與雲端 | 點對點已跨機器；共享頻道式 A2A（issue #28300 仍開著）與自動依賴排序沒有 ⟨G-10⟩ | 2026-09-19 |
| 把多 agent 工作流寫成能重跑的腳本 | 🧪 | Dynamic workflows（所有付費方案皆可用，Pro 要自己在 `/config` 開）；Claude Code Projects（public beta，僅 Pro／Max） | agent 撞到用量上限時該 agent 會暫停而不是失敗；Projects 還沒開放 Team／Enterprise ⟨G-02⟩ | 2026-09-19 |
| 它說做完了，但品質夠不夠 | 🧪 | Outcomes 規格驗證（beta，須帶 Managed Agents beta header）、`/goal`（check-ins 需 v2.1.234 以上）、`/code-review` | 答的是「做完沒」不是「寫得好不好」；社群的多模型對抗審查無公開對照數據 ⟨G-06⟩ | 2026-09-19 |
| 別家 agent 的 AGENTS.md，它讀不讀 | ✅ | v2.1.277（2026-09-18）起原生讀 `AGENTS.md` | 有 `CLAUDE.md` 時預設只讀 `CLAUDE.md`；要兩者並讀在 `/config` 設 `claude-md-and-agents-md`；Bedrock／Vertex／Foundry 讀不到 ⟨G-08⟩ | 2026-09-19 |
| 它需要你輸入時會不會叫你 | ✅ | 接上 Remote Control 可推手機通知、桌面閒置通知、`Notification` hook、`waitingFor` 可見性 | 社群補的是終端機標籤變色、實體燈號這類形式 ⟨G-03⟩ | 2026-09-19 |
| 把活分給 subagent、編排它們 | ✅ | Managed Agents（beta，須帶 beta header）、`/fork`／`/subtask`、subagent forking 預設開啟（v2.1.232） | 隊友可指名 `.claude/agents/` 的定義（#24316 已於 08-17 出貨），但 `skills`／`mcpServers` 不套用 ⟨G-01⟩ | 2026-09-19 |
| 它會不會跑出破壞性指令 | ✅ | 破壞性 git 指令自動封鎖（v2.1.183）、Sandboxing、`hard_deny`、worktree 隔離（v2.1.222） | git 層已完整；更廣的資源限制仍靠社群沙盒 ⟨G-04⟩ | — |
```

> **「Slack 裡要一個 AI 隊友」本波移出**（兩案皆同）：退場條款自測時它是 13＋1 列中唯一命中的——`## 時序` 從未有它的條目，開列日 2026-05-17 早過 90 天線，且本輪對官方一手只查到「文件索引仍列 claude-tag」、沒有變動。原 L62 自承「社群前驅稀薄、官方主導色彩強，列入僅供對照參考」與這個結果一致。Claude Tag 的事實家在 [[entities/claude-tag]]（本頁原本就以 wikilink 指過去），一字不失；移出於 `## 時序` 記一行（逐字見 A-9）。
>
> **〔併〕** 使用者裁「對照矩陣併入」時，在最上方 `❌` 區再插兩列（其餘 13 列一字不動），新表 15 列：
>
> ```markdown
> | CLAUDE.md 寫了它不聽 | ❌ | 無——截至 2026-09-19 官方文件未見；`/doctor`（v2.1.206）只會建議你把它寫短 | 沒有讓規則一定生效的官方機制；為什麼一直沒補見下一節 | 2026-09-19 |
> | 用 AI 寫久了技能退化、技術債變快 | ❌ | 無——截至 2026-09-19 官方文件未見，官方公開說法的方向與此相反 | 你能做的是自己量，見下一節 | 2026-09-19 |
> ```
>
> **「平台可及性（手機／瀏覽器）」兩案都不進新表**：它沒有任何社群工具或 issue 撐著（原表 2 的工具密度 ⭐⭐ 是全表最低），入口判準①與退場條款給同一個答案。保守預設下它留在 `## 對照矩陣` 節；〔併〕時它移出本頁，改由 [[entities/cowork]] 與 [[feature-radar]] 承接，並在 `## 時序` 記一行（逐字見 A-9）。它的細節家（技術彙整 L147–153）兩案都留原處。

## A-5 `## 目前結論`（節名保留，位置前移到結論表之後；取代 L167–179 內容）

> 採評審 🔴-4 第三案：`## 目前結論` 是 `.claude/reporter-rules/page-templates.md:87` 的 topics 模板必備節，且 `scripts/wiki_graph.py:62`、`wiki-lint-reporters/references/dispatch.md:37／45`、`wiki-lint-sweeps/references/sweeps.md:178` 三處以此節名派活——**不改名、不砍節**，只換內容並前移。

```markdown
## 目前結論

**官方為什麼還沒補**

上面的 ❌ 與「花費估不準」這一面，原因不是「還沒排到」，而是官方的動機本身指向別的方向。但它們說的是**到目前為止**的理由，不是預言——AGENTS.md 曾經也在這一節裡，2026-09-18 官方補了。

**花費為什麼還是估不準**：六個以上的成本監測工具（Tokenyst、CostHawk、Usage4Claude、Throttle Meter、Agent FM 等）反映的焦慮是真的。官方現在給得出歸因——`/usage` 能拆到哪個 skill、哪個子代理、哪個 MCP server 吃掉幾成，但給不出「這一次請求花了多少錢」，而且那些數字是這台機器的本機估算，不含你其他裝置與 claude.ai。官方的產品方向（讓 agent 自主執行更久）在結構上讓花費更難預估，6/15 信用池改制又加了一層。這不是技術做不到，是商業模式：用量變多對 Anthropic 有利。

**CLAUDE.md 寫了它不聽**：Writ（語意規則注入）、Caliber（跨工具設定統一）、Patina（腐化偵測）問的是同一件事——怎麼讓 AI 的行為邊界持久、可維護。官方對複雜 CLAUDE.md 幾乎沒有文件指引，`/doctor`（v2.1.206，2026-07-10）只會建議你把已提交的 CLAUDE.md 寫短，沒有碰到「規則被當成參考脈絡而遭略過」這個核心。

**用 AI 寫久了的副作用**：recap（技能退化）、modularity plugin（技術債加速）、AI 命名一致性（命名漂移）指的是長期隱憂。官方公開敘事（Cat Wu 的「AI 主動性」、Boris Cherny 的「數千子代理深度工作」）方向相反，承認這件事會削弱它自己的說法。

另外兩個 ❌（跨家 agent 的統一操作層、agent 之間的商業往來）不在這一節——它們太新，官方連表態都還沒有，沒有可寫的理由。

**你的選項**：這三件目前都得自己來——花費自己量（企業端的做法見 [[topics/enterprise-cost-management]]）、規則寫短並搭社群的注入工具、副作用靠自己定期回頭讀 agent 寫出來的東西。
```

> 保守預設下「CLAUDE.md 寫了它不聽」與「用 AI 寫久了的副作用」兩段講的是留在 `## 對照矩陣` 節的兩列，導言首句改：`上面的 ❌、下方對照表裡的兩列，加上「花費估不準」這一面，原因不是「還沒排到」⋯`

## A-6 缺口細節：要改寫的九條（其餘 ⟨G-nn⟩ 原文不動）

`**缺口細節**` 標題下第一行先加一句（評審 🔴-8）：

```markdown
> 下面提到的社群工具是當時的代表案例，不是現在的推薦；要裝哪個一律看 [[topics/community-tech-tools]]「我卡在這裡」。
```

```markdown
- ⟨G-01⟩ 把活分給 subagent、編排它們：2026-07-17 官方把「背景多開新 session」（`/fork`）與「同 session 委派子任務」（`/subtask`）拆成兩個指令；v2.1.232（08-13）讓 `subagent_type: "fork"` 預設繼承完整對話與 prompt cache。
  - 子項缺口已補：issue #24316（自訂 `.claude/agents/` 定義無法當隊友）官方 2026-08-17 關閉，答「spawning a teammate 時可指名 `.claude/agents/` 的定義，隊友照該定義的 `tools` 白名單與 `model`」。
  - 同一則官方留言明說 `skills` 與 `mcpServers` frontmatter 不套用到隊友。代表社群工具：Claude Squad、Harness（multi-worktree）；Boris Cherny 2026-05-13 揭露內部「數千子代理夜間跑批」工作流。
- ⟨G-02⟩ 把多 agent 工作流寫成能重跑的腳本：官方文件現在寫 Dynamic workflows「所有付費方案、Anthropic API、Bedrock、Google Cloud Agent Platform 與 Microsoft Foundry 皆可用」，Pro 要自己在 `/config` 的 Dynamic workflows 那一列打開——已不再標研究預覽。
  - 同一份文件寫：某個 agent 撞到你的 claude.ai 用量上限時，那一次執行會暫停而不是讓該 agent 失敗。
  - Claude Code Projects（public beta）讓一個對話協調多個平行雲端 session，目前只給 Pro 與 Max，Team 與 Enterprise 還沒有。
  - 手機遠端操控那條線已經收掉：issue #28322（`/rc` 不被辨識）官方 2026-08-19 關閉，答 v2.1.76 修好「Unknown skill」、v2.1.206 讓 `/remote-control` 一律解析得到。
  - issue #29006（Desktop 遠端控制 Claude Code session）2026-08-17 關閉，答遠端控制已可用，在「設定 > Claude Code > 預設啟用遠端控制」打開。
  - 代表社群工具：Gorchestra、TBD（agent-channels）、Superset。UltraCode 1.7M token 退款爭議官方沒有專文，這一輪查不到它現在算不算解決。
- ⟨G-03⟩ 它需要你輸入時會不會叫你：官方現在有四種——接上 Remote Control 後長任務結束或需要你決定時推手機通知（2026 第 16 週公告）、桌面閒置通知、`Notification` hook、`waitingFor` 可見性（v2.1.162）。社群工具補的是形式面：claude-needs-input（終端機標籤變色）、氛圍狀態燈（實體 LED）。
- ⟨G-05⟩ 新開一個 session 它就忘光：Claude Code 自己的官方記憶是 **auto memory**——依你的更正與偏好自己寫筆記，`/memory` 管理，子代理也各自維護一份，且跨 git worktree 共享。
  - Dreaming 是 Managed Agents（API 產品）那邊的功能，在 beta 之內還是更受限的研究預覽、須另外申請。
  - 要自己接外部記憶層：`PreCompact`／`PostCompact`（v2.1.76 起）／`SessionStart`／`SessionEnd` 四個 hook 官方確認都有，`SessionStart` 可回傳 `additionalContext` 把記憶注回來。
- ⟨G-05b⟩ 社群那邊沒有變少：ltm、VIR、CoreMem、OKF、OzBrain、ambient-context、mindmuxai/brain.md 各走不同路——OKF 跨工具跨模型，OzBrain 鎖團隊共享，ambient-context 走被動螢幕記錄，brain.md 走顯式寫入。
  - 它們解的不是 auto memory 解的那一題，所以 auto memory 上線後數量沒有下降。
- ⟨G-06⟩ 它說做完了，但品質夠不夠：官方的 Outcomes 規格驗證現在標 Beta，跟 Managed Agents 共用同一個 beta header；`/goal` 文件沒有預覽或 beta 標示，但它的 check-ins 要 v2.1.234 以上。`/code-review` 自 v2.1.218 起改在背景以子代理執行。
  - 這幾個答的是「任務條件成不成立」，不是「這段程式碼寫得好不好」。社群的 adamsreview、Mira、Read-Only Reviewer Agent 補的是後者，但「多模型對抗審查抓到更多真 bug」的說法雙方都沒有公開對照數據。
- ⟨G-07⟩ 額度快用完、錢花到哪看不看得到：官方 `/usage` 現在會拆到 skills、子代理、外掛與個別 MCP server 各佔幾成，標出佔比 ≥10% 的行為旗標，也給每個 loop 的 token 數；狀態列可讀 `rate_limits.five_hour.used_percentage` 與 `seven_day` 自己做顯示。
  - VS Code 另有額度警示橫幅，門檻 70%；企業端有 Spend Controls（2026-07-04 宣布，控管粒度未公開，見 [[topics/enterprise-cost-management]]）。
  - 官方自己註明：這些數字是從這台機器的本機 session 紀錄算出來的近似值，不含你其他裝置或 claude.ai 的用量。
  - 還缺的比原先寫的窄：沒有逐次請求的金額，也沒有主動推到手機或桌面的額度告警、門檻似乎不能自訂（官方文件未見此設定，不是官方明說沒有）。代表社群工具：LimitBar、CCLimitPing。
- ⟨G-08⟩ 別家 agent 的 AGENTS.md，它讀不讀：**已補**。Claude Code v2.1.277（2026-09-18）起可直接把 `AGENTS.md` 當專案指示讀，官方文件寫「works without adding a `CLAUDE.md`, an import, or a setting」。
  - 三個邊界：有 `CLAUDE.md` 或 `CLAUDE.local.md` 時預設只讀 `CLAUDE.md`；要兩者並讀，把 `/config` 的「Project instructions」設成 `claude-md-and-agents-md`。
  - Bedrock／Vertex／Foundry 或關掉遙測的 session 讀不到，那些場合仍用 `@AGENTS.md` import。issue #6235 官方已於 2026-08-17 關閉，當時給的是 import／symlink 做法，原生支援是一個月後的事。
- ⟨G-12⟩ Agent 跟 agent 做生意、吵架怎麼判：internet-court／internet-court-skill（2026-08-18 首見）提出 agent 對 agent 商業往來的信任層——自然語言協議＋ERC-7710 委任權限＋x402 支付＋履約爭議仲裁；官方目前沒有任何對應功能或路線圖線索。
  - 2026-09-19 查過這個專案本身：5,855★、106 forks，但只有 9 位 watcher、4 筆 commit、1 位貢獻者，2026-06-16 建立、2026-08-19 之後沒再更新。星數撐不起「已被採用」——這一列先擺著標出缺口存在，等更多獨立工具出現再談證據強度。
- ⟨G-13⟩ 想自己決定哪段用哪個模型、不被鎖住：個人端官方只有 `opusplan` 這一種固定切換——規劃模式用 Opus，進到執行換 Sonnet。org default model（v2.1.196，2026-06-29）與 `enforceAvailableModels`（v2.1.175）都是企業管理端的設定。
  - 依成本或任務動態挑模型，截至 2026-09-19 官方模型設定文件未見。代表社群工具：Workweave Router（HN 181，實測降 40%+）、Dragoman、Council、Ungate、Rayline。
  - The Information（2026-09-15）與 Dealroom（2026-09-16）均報導開發者正把 Claude Code 接到非 Anthropic 模型後端執行，兩則都沒有規模數字。
```

> ⟨G-05b⟩ 與 ⟨G-13⟩ 是新編號（原 ⟨G-05⟩ 一條 393 字元超過細節區 200 字元上限；原 L60 那一列從來沒有短標記，社群起源與代表工具無處可去）。兩者都只新增 `⟨G-nn⟩` 序號，**不是懸置標記**。

## A-7 懸置細節（L91–92）：⟨Q-02⟩ 結案

原 `**懸置細節**` 區只有 ⟨Q-02⟩ 一筆。依 verified 六-8（`gh api repos/internet-court/internet-court-skill`，2026-09-19），改寫為事實並移除標記——事實已寫進上方 ⟨G-12⟩ 第二條，故**整個「懸置細節」區與 L92 一併移除**，新表該列不再帶 `❓ 待查證 ⟨Q-02⟩` 短標記。

## A-8 相關實體（取代 L183–187）

```markdown
- [[topics/community-tech-tools]] — 該裝哪個社群工具，答案只在這裡（「我卡在這裡」症狀決策表）
- [[topics/anthropic-agent-stack]] — 官方的 agent 積木各自為什麼出、怎麼疊起來
- [[topics/community-tech-patterns]] — 社群拿這些工具玩出哪些做法、哪些已經站住腳
- [[topics/community-tech-discussions]] — 社群在吵的觀念問題（「該不該這樣用」；本頁答的是「有沒有這個功能」）
- [[feature-radar]] — 這禮拜官方動了什麼、值不值得現在跟
- [[entities/claude-code]] — Claude Code 現在有哪些毛病、哪些修了，以及每個 issue 的留言與讚數
```

## A-9 時序（prepend 一則，與原 L191–192 的 2026-09-19 段合併）

```markdown
### <實作當天>
- **AGENTS.md 由缺口轉為已補**：Claude Code v2.1.277（09-18）起原生讀 `AGENTS.md`，本頁該列自 ❌ 改 ✅ 並補三個邊界；同時更正 08-17 官方一次關閉的五個 issue（#6235、#24798、#47023、#24316、#29006）與 #14227（05-25 標為不打算做），本頁原先仍當它們是進行中的社群壓力。
- **「通知」與「額度監控」兩列低估官方**：手機推播（2026 第 16 週）、桌面閒置通知、`Notification` hook 使前者改 ✅；`/usage` 的歸因拆解、狀態列 `rate_limits`、VS Code 70% 警示橫幅使後者的缺口收窄為「沒有逐次請求金額、不含其他裝置、告警不能自訂」。
- **「跨 session 記憶」比錯了對象**：Claude Code 使用者的官方記憶是 auto memory，不是 Managed Agents 的 Dreaming；四個 session／compact hook 官方確認都存在。
- **另有四列狀態更正**：Dynamic workflows 已開放所有付費方案、不再是研究預覽（另收 Claude Code Projects public beta）；Agent view 是研究預覽而非「官方無對應」；Outcomes 為 beta；個人端模型切換官方只有 `opusplan`。
- **Cowork 網頁／行動版的範圍問到了**：官方發布日為 2026-07-07、用詞是 beta access（先開放 Max），且明說桌面才是完整體驗——網頁與行動版用不到你本機的檔案與瀏覽器。本頁原記的「正式擴展（07-08）」與「功能範圍是否等同桌面待觀察」兩點就此結清。
- **internet-court-skill 的採用程度查過了**：5,855★、106 forks，但只有 9 位 watcher、4 筆 commit、1 位貢獻者，2026-08-19 後未再更新；星數不足以當作被採用的證據，該列改為據實記載。
- **「Slack 裡的 AI 隊友」這條線移出本頁**：它自建列起就沒有社群前驅在推，官方也沒有新動作；Claude Tag 本身的事實見 [[entities/claude-tag]]。
- **同一個痛點不再有兩個互相打架的狀態**：先前「多模型路由」一處寫「官方無對應」、另一處寫「部分對應」，跨 session 記憶也有兩個狀態；現在每個痛點只有一個狀態，並標出最後一次對過官方的日期。
```

> **〔併〕** 時再加一則：`- **手機與瀏覽器能不能用，改看產品頁**：這條線的內容是官方產品動態而非社群痛點，完整記錄見 [[entities/cowork]] 與 [[feature-radar]]。`

**懸置標記數變動（保命條款，須寫進 commit 訊息）**：本頁原 2 筆 → 0 筆。⟨Q-02⟩ 依 verified 六-8 查證結案（`gh api` 數據已寫進 ⟨G-12⟩）；L153 Cowork 依 verified 六-5 官方 blog 結案（日期 07-07、beta access、桌面才是完整體驗，已寫進新表第 12 列或對照矩陣列與時序）。全庫 188 → 186，基線 140，未低於基線。實作時跑 `python scripts/check_pending_markers.py --rebuild-count --reason "第 14 波 official-community-gap：⟨Q-02⟩ 與 Cowork 兩筆經官方一手查證結案"`。

---

# B. 進規則檔

## B-1 `.claude/reporter-rules/features/pages.md`：取代現行 L154–156「topics/official-community-gap」整節

```markdown
## topics/official-community-gap

### 產品化矩陣同步（「官方補了沒」表）

一張表答一個問題：社群喊的痛，官方補了哪幾個、哪幾個還沒補。**覆寫式**，五欄固定：你痛的是什麼｜官方補了沒｜官方給的是什麼｜還缺什麼、你現在能怎麼辦｜核對日。

**狀態三值，不引進第四值：** `✅` 官方有正式功能可直接用／`🧪` 官方有東西但受限（測試中、須申請、只給企業端）／`❌` 官方目前無對應。判定一律以官方一手為準（官方文件、CHANGELOG、issue 的官方回覆）；頁面自己的舊記錄不得用來反駁一手。`❌` 要寫「截至 YYYY-MM-DD 官方文件未見」，把否定證明的範圍講出來，不寫成無條件的「沒有」。

**入口判準（三條全過才開新列，只管新列、既有列的去留看退場）：** ① 這個痛有 ≥2 個獨立社群工具，或一則 GitHub issue ≥100 個讚；② 痛的對象是 Claude Code 或 Anthropic 產品的行為；③ 寫得出「官方補到哪」——查得到官方對應，或已確認官方確實沒有。

**上限 18 列**，依狀態排序（`❌` 在最上、`✅` 在最下）。**表滿時不硬塞**：新列先與最下方的 `✅` 列比「哪一個還有人在痛」，輸的那列依退場條款移出並於 `## 時序` 記一行；兩者都還在痛就把上限議題寫進 `wiki/log.md`，不自行擴表。

**退場，只有一條：** 一列的去留看**還有沒有人在痛**，不看官方功能還在不在跑。判斷用兩個都查得到的訊號，**兩個都成立才移出**：① 該頁 `## 時序` 最後一次提到這一列是 90 天前或更早（時序從未提過的，從開列日起算）；② 這一輪 5n 對官方一手也沒查出變動。移出時於 `## 時序` 記一行，並把該事實仍住在哪一頁寫出來。**唯一例外：** `❌` 列不因天數移出——「官方到今天還沒補」本身就是結論。

> 為什麼用 `## 時序` 當時鐘：它是本頁唯一每次改列都會留痕、且 grep 得到的社群側紀錄。用「核對日」當時鐘會讓 5n 每週自己把鐘撥回去，等於永不退場；用「官方功能還在不在」當條件，則每一個 `✅` 列都永遠留著——兩者都是本波要治的病。

**兩個事實不住這張表：**
- issue 的留言數與讚數住 [[entities/claude-code]]；本表只寫 issue 號與它開著還是關了（附關閉日與官方怎麼答）。
- 代表社群工具住 [[topics/community-tech-tools]]「我卡在這裡」決策表；本表不列工具名，歷史上的代表工具與社群起源日期留在該頁「缺口細節」的 `⟨G-nn⟩` 條目，且該區開頭要有一句「這些是當時的代表案例、不是現在的推薦」。

**三條進料：**

| 進料 | 節奏 | 觸發邊 |
|---|---|---|
| 官方發布新功能 | 每日 ingest | 類別路由：判定為功能的條目，檢查表上有無對應列；有則更新「官方給的是什麼」「官方補了沒」與「核對日」；無對應列則照入口判準判，判不進表的在 `wiki/log.md` 記一行理由 |
| 社群記者轉知的新痛點 | 每日 ingest | `.claude/reporter-rules/community/daily.md` 的轉知旁路（`scripts/pending_handoffs.py`）；同一組入口判準，處置同上 |
| **拿整張表對官方一手** | 🗓️ 週更，`/wiki-lint` 5n | **本節即其觸發邊**——類別路由接不到「官方悄悄補了但沒上日報」這件事。2026-09-19 查證時 14 列中至少 5 列低估官方，且錯的方向一致，根因就是這一欄沒有人定期對一手 |

### `/wiki-lint` 5n：整張表對官方一手（主編親做）

記者無 web 工具，這一步只能主編做。逐列：

1. 表上每個 issue 號只取狀態與官方回覆，不要把整包留言倒進來（#6235 有 405 則）：
   ```
   gh issue view <號> -R anthropics/claude-code --json state,stateReason,closedAt,comments \
     -q '{state,stateReason,closedAt,official:[.comments[]|select(.authorAssociation=="COLLABORATOR" or .authorAssociation=="MEMBER")|{login:.author.login,body:.body[0:400]}]}'
   ```
2. 抓 CHANGELOG 全文再本機 grep 該列關鍵詞：`gh api repos/anthropics/claude-code/contents/CHANGELOG.md -H "Accept: application/vnd.github.raw"`。
3. 官方文件站有對應頁者一併看（code.claude.com／platform.claude.com；否定證明查 `code.claude.com/docs/llms.txt` 的文件索引）。
4. 核完更新該列「核對日」；核對日逾 30 天的列列進 lint 回報。

回報行：`官方補了沒表：N 列比對／M 列已改／核對日逾 30 天 K 列`

> **機械看守：無。** 沒有任何腳本在看「核對日」這一欄——`scripts/check_wiki_freshness.py:64` 只看頁面層新鮮度，`scripts/check_cell_limits.py` 只看字元數，`scripts/check_pending_markers.py` 只看懸置標記。最小看守方案見 `docs/page-audits/ledger.md` 機制待辦（`scripts/check_verify_dates.py`，帶存量基線、逾 45 天 WARN／90 天 FAIL）。在它落地之前，這一欄只靠 5n 這條人工步驟。
```

## B-2 `.claude/skills/wiki-lint-sweeps/SKILL.md`：五處字面＋一列

十三步表加一列：

```markdown
| 5n | official-community-gap「官方補了沒」表對官方一手 | 主編親做 |
```

同檔五處逐字改（`grep -n "十三"` 實測命中，行號為現行檔）：`:3` description「十三個週更掃描（5a–5m）」→「十四個週更掃描（5a–5n）」、`:7` H1「（5a–5m）」→「（5a–5n）」、`:9`「這十三步」→「這十四步」、`:13`「## 十三步」→「## 十四步」、`:43`「十三行回報」→「十四行回報」。
「順序與相依」加一句：`- 5n 與 5a 同批看官方 CHANGELOG：5a 抓下來的全文可直接給 5n grep，不必抓兩次。`
「邊界」的探測句加 5n：`雲端執行 5b／5c／5m／5n 前先跑 egress 探測`（5n 吃 GitHub，走 `--group github`，與 5m 同組）。

> `.claude/skills/wiki-lint-rules-health/SKILL.md:236` 的「十三行回報（6a–6l…）」是 **C 段**的計數，**不得一起改**。

## B-3 `.claude/skills/wiki-lint-sweeps/references/sweeps.md`

1. `:1` H1 →「# Wiki Lint B 段：5a–5n 逐步判準與回報格式」
2. 新節標題用 `## `（與既有各步同級），逐字：`## 5n. official-community-gap「官方補了沒」表對官方一手（主編親做）`
3. 節內容＝B-1「`/wiki-lint` 5n」的四步與回報行逐字搬一份（該檔是各步判準的家，SKILL.md 只列表不重述）；並寫明雲端走 `python scripts/cloud_egress_check.py --group github` → `EGRESS: github OK` 才跑，`PARTIAL`／`BLOCKED` 跳過並寫待辦（與 5m 同形，滿足 registry 第 101 組 egress 契約）。

## B-4 `.claude/skills/wiki-lint/SKILL.md`、runbook 與 registry（評審 🔴-3）

1. `.claude/skills/wiki-lint/SKILL.md:15` →「`5a–5n` 主編親做／親查的十四個掃描」
2. `docs/cloud-runbooks/weekly-lint.md` 步驟表加一列，標題與 sweeps.md **逐字相同**：
   ```markdown
   | `wiki-lint-sweeps` `5n. official-community-gap「官方補了沒」表對官方一手（主編親做）` | **先探測再決定**：`python scripts/cloud_egress_check.py --group github` → `EGRESS: github OK` 就跑 `gh issue view`／`gh api`；`PARTIAL`／`BLOCKED` 才跳過並寫待辦 |
   ```
3. `.claude/review-registry.json` 第 113 組（`雲端 weekly runbook <-> wiki-lint-sweeps/references/sweeps.md：B 段步驟標題錨點逐字相符`）的 `patterns` 陣列加一筆 —— **用短前綴 `"5n\\. official-community-gap"`**，與同組既有寫法（`"5e\\. pricing"`、`"5m\\. code-quality-decline"`）一致；該組 `_note` 明載「新增步驟時必須同步加進本 patterns」，只加 sweeps.md 不加 runbook 與 pattern，`all_contain` 仍會全綠（它只驗「清單裡的都在」）。
4. 動 `.claude/` 規則檔前先讀 `.claude/rules/claude-md-edit.md`；改完跑 `python scripts/check_rules.py`（改動前為 ✅，不得轉紅）與 `python scripts/run_tests.py`。

---

# C. 兩閘自查

見 proposal 第二輪節末兩行。
