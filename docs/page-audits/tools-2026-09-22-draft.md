...
> 每一列最後一次確認是哪天，寫在「證據」欄的**查**；工具出資安事故、棄坑、下架這種急事，見 [[entities/claude-code]] 與 [[topics/ai-agent-safety]]。
...
| 額度快用完，想在斷線前被提醒 | ⌨️ [**Claude-Code-Usage-Monitor**](https://github.com/Maciek-roboblog/Claude-Code-Usage-Monitor) | 要工作列常駐、想自己設幾 % 就叫你 → 🖥️ [usage-monitor-for-claude](https://github.com/jens-duttke/usage-monitor-for-claude)（僅 Windows）；額度一解封就自動接著跑 → ⌨️ [CCLimitPing](https://github.com/wavever/CCLimitPing) | ⚪（查 09-22，8,713★） |
...
- **多 agent 互踩**：ness 09-22 查得 99★／13 forks，自述已從 CLI 變成「給 agent 用的 IDE」，但 README 仍寫可同時跑十個 Claude，仍解同一個症狀，故不換首選。另有一個同名的 `revfactory/harness`（9,051★）是設計 agent team 的 skill，不是它。
...
- **額度告警：官方只能被動查**。`/usage` 看得到用量條、狀態列讀得到百分比，但不會主動叫你；官方 CLI 主動告警的需求（issue #13585，👍 124）到 09-22 仍未處理，另一條同類需求 #65292 已被官方標為不做。官方功能進度見 [[feature-radar]]。
- **額度告警這一列三個工具都只有星數**：Claude-Code-Usage-Monitor 8,713★ 但 2026-07-05 後未更新；usage-monitor-for-claude 293★、09-13 仍在更新但只有 Windows；CCLimitPing 45★，解的是「解封自動接著跑」不是提醒。三者都未見第三方實測。
...
**（6）額度告警那一列的三個工具，插在目錄最上方**（首次出現 2026-09-22，倒序後自然排在最前）：
...
```markdown
| [**Frugal Tokens**](https://github.com/dpclark4/frugal-tokens) |   ← 只換連結，其餘四格不動（L201）
| [**Graft**](https://github.com/trailhq/Graft) |                      ← 只換連結，⚠️ 與其餘四格不動（L202）
| [**Workweave Router**](https://github.com/weave-os/router) |         ← 只換連結（L213）
```
...
```markdown
  （改名、轉手、換網域、星數增減、長期沒人回訪**都不算**否定證據，只更新描述與連結；**專案自述或 README 不再宣稱它解這個症狀**才算——2026-09-22 的 Omar 即屬此類：🟢 背書的「TUI 管 100 個 agent」在一手已不存在，證據降為 🟡 而首選不換。）
```
...
```markdown
4b. **可達性**：目錄列一律要有可點的連結（repo、產品頁、示範站或原始貼文擇一）。第 2 條的排除條款（「無公開 repo / demo / 連結」不收錄）**回頭適用於存量列**：發現無連結的既有列，先花一次查證補上連結；補不上就移除該列（git history 留存，且多數在 `wiki/log.md` 或 archive 頁另有出處，移除前逐列確認過再動）。決策表首選／次選的目錄列照第 4 條豁免，但要在簡介末寫一句「YYYY-MM-DD 查不到公開頁」，不留白裝正常。2026-09-22 第 15 波依本條一次清掉 43 列（152 → 112）。
...
**取代「community-tech-tools 策展規則」第 3 條末的「廣泛採用才給 `✅`」一句：**

```markdown
   - 採用預設 `⚡`；`✅` 要有**採用的證據**而不只是規模：至少兩筆獨立的實際使用回饋（他人文章、Reddit／HN 留言、明確的下游依賴），或 forks/stars 比例與活躍 issue 顯示真有人在用。只有星數一律不給 `✅`。給 `✅` 的列在簡介裡寫出那兩筆證據是什麼。既有 `✅` 在每輪被拿去對一手時一併重判（2026-09-22 即依此把 Harness 99★、Omar 48★ 從 `✅` 降為 `⚡`，Claude Usage Widget 因無法辨識是哪個專案而整列移除）。
```

## B-4 `.claude/reporter-rules/community/weekly.md`：驗收自檢加第 4 題

**放在「community-tech-tools 決策表驗收：3 跳自檢題」的第 3 題之後，並把節名的「3 跳自檢題」括號現名改為「4 題」：**
