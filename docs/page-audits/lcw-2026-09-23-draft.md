...
> L31 拿掉「每週從 patterns 沉澱一次」（冷讀者內部用語表逐字點名 `:31`），改用 `page-templates.md` 週更頁的標準寫法。L34 填 2026-09-16＝本頁收進的最新社群料（hcom，patterns L301）。frontmatter L8 由 `scripts/gen_wiki_frontmatter.py` 從 L31 重產，不手填。

## A-2 callout（取代 L36–L37）

```markdown
> **四面牆，官方都已給了起點**（<實作當天>）
> 每個 agent 一個 worktree、腳本呼叫加 `--bare`、跨 session 的 auto memory、要 Claude 出示證據才算做完——這四件官方都已內建，而 Claude Code 一個 session 預設最多同時跑 20 個 subagent，那是上限，不是「20 個都跑得穩」。四條線的答案因此都改成先用官方零件、社群工具補在上面。
```

> 兩句（`weekly.md:107` callout ≤2 句）。寫的是讀者拿到的新事實（官方 20 上限、四件官方零件），不寫「本頁改版」。
...
```markdown
**現在的答案**
- **官方已給**：腳本或 SDK 呼叫加 `--bare`，跳過 hooks、skills、MCP server、auto memory、CLAUDE.md 等自動載入（[官方文件](https://code.claude.com/docs/en/headless)）；`/context` 看得到啟動時載入了什麼，單次讀取量官方也已設上限——見 [[topics/coding-workflow-guide]]
...
| 子問題 | 社群走到哪 | 代表實作 | 證據強度 |
|---|---|---|---|
| 按需取回 | 預先 @-mention 定為反模式；索引層；不裁剪會 O(N²)（62.8–85.9% 額外 token）；nightshift 疊三層因應多 issue context rot | Just-in-Time Retrieval、Compact Memory、nightshift 等，見 [[topics/community-tech-patterns]] | 多來源 |
...
```markdown
**現在的答案**
- **官方已給**：要 Claude 出示證據（測試輸出、跑過的指令）而不是宣稱完成，再用 `/goal` 或 Stop hook 把「做完沒」的裁決權從執行者手上拿走——見 [[topics/coding-workflow-guide]] 第 9 段
...
⟨Q-01⟩⟨Q-03⟩ 併成「fork 子代理」一筆（同一官方頁、同一結論，健檢卡第 7(2) 節 #6）；⟨Q-02⟩ 內容照留，只把「已查證（…，查證日 2026-09-20）」改寫成「查 …（2026-09-20）」。三個短標記全部拿掉，節名「懸置細節」改「查過的數字」（冷讀者內部用語表 `:70`）。
...
- **與官方做法頁說法不同的兩處**：[[topics/coding-workflow-guide]] 不再推薦「自己加讀取上限」（官方已內建上限）與「已否決方案索引」（對應官方 issue 已關閉、零回應），社群仍有人在做或在要；分界是那頁只收官方機制撐得起的做法、本頁記社群走到哪，所以兩條只留在子問題表。
...
%% 週更撈料：patterns 節點以 `**主線：**` 欄位標記所屬線（規則：.claude/reporter-rules/community/daily.md「主線 tag 規則」、weekly.md「週更整線重寫」） %%
...
全部在 `.claude/reporter-rules/community/weekly.md`「## community-large-codebase-workflow 週更整線重寫」（L79 起），B-6 在 `community/daily.md`。`review-registry.json:666–675` 要求 daily.md 與 weekly.md 都含 `community-large-codebase-workflow` 與 `**主線：**`——B 區沒有刪到任何一處。

## B-1 骨架第一層（`weekly.md:89` 整行換掉）

```
   **現在的答案**            ← 首條「**官方已給**：…」連頁不連錨（官方逐字只用已查證的句子）；其後 ≤ 3 條社群可執行結論
```
...
```
1. **撈料**：`Grep "\*\*主線：\*\* [^—]" wiki/topics/community-tech-patterns.md`，取本頁「最後更新」日之後帶 tag 的節點（不用固定 7 天窗——兩次重寫間隔超過 7 天時，中間的節點會漏掉）；另掃同期填 `—` 的節點，照 `.claude/reporter-rules/community/daily.md`「主線 tag 規則」的判準與反例重判一次。代表實作的名字與數字若已不在 patterns 本體（月度封存搬走），去 [[topics/community-tech-patterns-archive]] 找
```
...
```
- 容易誤判成 `—` 的反例：多 agent 協作、編排、互相傳訊、commit 落地排序的工具，即使不限 monorepo 也填「並行規模」——大 repo 最先撞到的就是這一面牆；只解計費、裝置存取或單一 agent 使用體驗的才填 `—`
```
...
```
python scripts/pending_handoffs.py open --from 社群 --to 功能 --page entities/claude-code --note "L502 寫 v2.1.233（2026-08-14）『新增 --worktree 旗標』，同頁 L821 記 2026-05-08 v2.1.133 已可控制 --worktree、EnterWorktree——兩句互斥（第 16 波冷讀者『撐不起的句子』第 5 條）。請對 v2.1.233 release note 原文核對新增的是哪一個 --worktree 相關項目。"
```

C-5（社群：cqd 與 discussions 的分界）

```
python scripts/pending_handoffs.py open --from 社群 --to 社群 --page topics/code-quality-decline --note "第 16 波冷讀者分不出本頁與 community-tech-discussions：discussions L67／L77『社群已確認根因不在模型退步』vs 本頁 L113–L118『三種解釋都排除不掉』、L157『不是同一件事』卻沒說差在哪。主線頁線 2 已寫成『同一 session 越跑越笨見 discussions，換版本後變差見 code-quality-decline』；請確認這個分界成立，並在兩頁各寫一句讀者看得到的分界。另：本頁 L78 🧰 行把『先量 context 組成』送回主線頁線 2，主線頁現改指官方 /context（經 guide），循環已從主線頁那側解開；本頁 L78 是否改寫由維護者判。"
```
...
**同維護者、本波不改、下次該頁策展處理**（`community-tech-tools:213` loopx 仍 4,476 星且連舊 owner `huangruiteng`，verified 為 5,925★、已轉址 `loopx-project/loopx`；`community-pattern-trends:37` 有「本輪」、`:99` 標題寫「第三波」）——只記 log，不開轉知（同一維護者）。
...
0. **B 區先做**（B-1～B-6）。驗：`python scripts/check_rules.py` 綠；`grep -c "官方已給" .claude/reporter-rules/community/weekly.md` ≥ 2；`grep -c "容易誤判成" .claude/reporter-rules/community/daily.md` ＝ 1。
1. A-1 標頭、A-2 callout、A-3 摘要與摘要表兩列。驗：全頁 grep「沉澱」「節點」「本輪」＝ 0（`%% %%` 內除外）。
2. A-4～A-7 四線，標題與「為什麼會這樣」四行不動。驗：四個 `### N.` 標題與原文 diff 為空；四條 🧰 行七個「」症狀句逐字與原文相同。
3. A-8、A-9。驗：全頁 grep `⟨Q-` ＝ 0。
4. 跑閘：`check_tools_page.py` → `check_cell_limits.py --page topics/community-large-codebase-workflow` → `check_reader_language.py --page topics/community-large-codebase-workflow` → `check_pending_markers.py`（119 筆不變）→ `gen_wiki_frontmatter.py` → `check_hierarchy.py` → `build_web.py`。
5. C 區五筆轉知、`wiki/log.md` 一筆條目、`python scripts/run_tests.py` exit 0。
