...
| 根目錄 `ObsidianLab/.gitignore` 的 `logs/` 規則（無前導斜線，比對任意深度）意外連帶忽略了 `CLAUDE_NEWS/src/logs/task_scheduler.log`——該規則原意是排除本機開發用的 log 目錄（註解「keep locally, don't commit」），但雲端 routine 的 Step 6／中止落地流程明文要求此檔必須 commit + push（容器銷毀後本機保留無意義）。2026-07-25 本次執行 `git add src/logs/task_scheduler.log` 時被 git 擋下（`The following paths are ignored`），改用 `git add -f` 強制加入單一檔案完成本次 push | 在根目錄 `.gitignore` 為此檔加白名單例外（`!CLAUDE_NEWS/src/logs/task_scheduler.log`），或將 `logs/` 規則改為錨定路徑（如 `/logs/`）避免誤傷子目錄同名資料夾；擇一修好後移除雲端 routine 每次執行時的 `-f` 強制加入步驟 | 使用者（決定 gitignore 修法） | 2026-07-26 | 🟡 繞路中（已用 `git add -f` 完成本次 push，非長期解） |
