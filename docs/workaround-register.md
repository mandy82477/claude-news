...
| **官方頁監看 state 的持續成本**（2026-08-29 段落級 diff 上線附帶）： 由數 KB 增為 210 KB（存 1631 段官網原文供比對），且每次抓料 commit 都會重寫。存原文是「說得出移除了哪一段」的必要條件，但這是先前沒被算過的持續成本 | 觀測三個月的 repo 體積成長；若成為負擔，可只對「移除偵測」保留原文、其餘改存段落雜湊（代價是移除時只說得出「少了 N 段」說不出內容）。 只擋單頁上限，不擋檔案總量 | Claude | 2026-11-29 | 🟢 已知取捨，觀測中 |
