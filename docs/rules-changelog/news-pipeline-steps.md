...
## 2026-09-25 日報跨日重複：快取過期根因＋聚焦跨日閘

使用者：「今天日報內容跟前幾天有點重複」「今日頭條你也審視一下」。

**量測：** 09-15～09-24 三十條聚焦裡 9 條是前幾天登過的故事（Suleyman 09-16／17、入侵 OpenAI 09-18／19、Accenture 與反壟斷訴訟 09-19／20、Chat 與 Cowork 合併 09-17／21、AGENTS.md 09-19／21）；重點話題的 GitHub issue 大多是老 issue，#6235 七月以來上 11 次、#34255 上 9 次，約兩週一次。

**根因一（程式，`emitted_cache.py`）：** 快取條目依 first_emitted 滿 14 天就刪，GitHub Issues 以 updated_at 抓，老 issue 天天被抓回，第 15 天即被當新條目；09-15 改內容雜湊鍵又讓舊裸 URL 條目對不上，09-16 一天重登 10 則。改依 last_seen 過期、裸 URL 條目代表內容鍵，補種 119 個歷史 issue。回放 09-13～09-24：GitHub issue 64 → 16。細節見 commit 14c9e4b5。

**根因二（選材，Step 1b）：** 爬蟲去重只合併同一 URL 與同日相似標題，跨日同一故事換一家媒體就是新 URL；選材規則也沒有「前幾天登過」這個維度，[持續追蹤] 被當成熱度標籤用（09-19 AGENTS.md 上線標 [持續追蹤]、09-15 單一來源也標）。
