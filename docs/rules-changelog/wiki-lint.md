...
**2026-08-28 A**（lint 回報格式缺「轉知處置」欄）：`.claude/reporter-rules/shared.md` 的回報契約要求記者每次都回報轉知處置，而 lint 派工訊息確實會附上轉知待接手清單（見步驟 2）——但 lint 專用回報格式原本沒有這一欄，記者的處置無處可寫、主編也無從核對，帳本結不了案。兩份規則對同一行為給出不同格式，本身就是 6a 該抓的矛盾型態。
