...
## 2026-09-13（callout 規則從禁止清單改回一句定義）

**起因：** 讀者版日報改為各頁頂部 callout 的投影（沿革見 `docs/rules-changelog/news-pipeline-steps.md` 2026-09-13 丙）後，Sonnet 冷讀者評審連跑五輪（6→7→6→7→8 分）。每輪抓到的都是同一種病：規則寫「禁止 X」，記者做出「不是 X 但一樣壞」的東西——禁 `⟨G-11⟩` 與「見『## 節』」，就出現引號包小標題「見『IPO 走到哪一格』細節區」；要「補一句判斷」，就補出「尚無定論」「來源仍是單方」；要「不重述事件」，就把主詞也刪了（投資訊號判讀 09-12 只剩「負面、看十月」）；兩個人物頁共用同一句罐頭判斷。主編每輪往 callout 段再疊一句禁止，最後那段變成一長串「不要」，真正的判準（陌生人看得懂嗎）被埋在句尾。

**使用者裁決：** 「太複雜了，先從源頭解決：callout 要產生的東西是給讀者看的，需 summarize 今日重點。」

**做法：** callout 段重寫成一句定義＋五條外殼規則。定義：callout 是寫給讀者版日報的，日報一字不改地抄它，讀者只看到這一段——所以它就是這頁今日重點的摘要（發生了什麼、本頁角度、對讀者意味什麼），自檢是把整頁遮掉只讀這段。原本累積的禁止項（頁內編號、路標、套話判斷、重述事件、日期寫事件日）全部降級：外殼、日期、單則、同一事件一個家、位置各一條；產生器的 WARN 保留當兜底，規則裡只用一句註明「那些是陌生人看不懂的症狀，不是規則本身」。

**主編派修正任務時的同款教訓：** 給記者「改成這個樣子」加一個範例，不要列「不要這樣」——投資訊號那條就是「不重述經過」被字面執行的結果。

## 2026-09-23 拆頁判準：行數由「鬧鐘」升為「加分訊號」

使用者原話「行數也是可以考慮啦」。`page-lifecycle.md`「一頁一故事」與 `.claude/skills/page-audit-review/SKILL.md` 原則 3 同步：蒸餾後仍 > 600 行且考題集分得出群，厚度可直接列為拆子頁候選的加分訊號，不必等「有人問全站答不了／答案會一直變／同一事住三頁」三訊號同時亮。不按行數對半切、不按月份切、不按層切三條不變。背景：第 10–17 波拆子頁候選連續判 0，三訊號門檻偏嚴。
