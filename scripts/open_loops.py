...
INQUIRY_LAMP_DAYS = 21  # 與 .claude/skills/wiki-lint-reader-acceptance/references/inquiry.md「人類質疑時效燈」同步
...
def reader_notes_pending(today: date) -> list[str]:
    ...
    `.claude/reporter-rules/features/daily.md`「⏳ 觀望是有期限的判斷，不是停車場」）。
...
def feature_radar_watching(today: date) -> tuple[int, int]:
    ...
    只有「逾 90 天」那個數字進總計：依 `.claude/reporter-rules/features/daily.md`
