...
語法規格見 `.claude/reporter-rules/page-templates.md`「懸置標記語法」節，解析邏輯全部
...
檢查項對照 `.claude/reporter-rules/page-templates.md`「懸置標記語法」節：
...
def check(report: list[str], wiki_dir: Path | None = None, today: date | None = None,
    ...
    if baseline is not None and total_legacy > baseline:
        ...
        report.append(
            ...
            f"新標記請直接用新語法（見 .claude/reporter-rules/page-templates.md「懸置標記語法」）——"
