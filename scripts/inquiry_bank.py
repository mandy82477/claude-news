...
為什麼存在（2026-09-02）：
    ...
    流程見 .claude/skills/wiki-lint-reader-acceptance/references/inquiry.md「題庫維護」）。
...
BANK = [
    {
        ...
        "probe": [
            ...
            "答不出來源 → 依 .claude/reporter-rules/page-templates.md 懸置標記語法標記，或退回對應記者",
        ],
    },
    {
        "id": "Q2",
    ...
    },
    {
        ...
        "probe": [
            ...
            "feature-radar「現在值得跟的三件」最後輪替日 >14 天 → 依防霸榜規則覆核",
        ],
    },
    {
        "id": "Q4",
    ...
    },
    {
        ...
        "probe": [
            ...
            "跑儲存格量測（>120 字元即違規，指令見 .claude/reporter-rules/page-templates.md「表格放結論，細節下沉」）",
            "讀該頁前 160 字：能否不看背景就懂（delta-first ＋ 可獨立閱讀）",
            "違規依 .claude/reporter-rules/page-templates.md 修復；修不動記待辦",
        ],
    },
    {
        "id": "Q6",
    ...
    },
    {
        ...
        "probe": [
            ...
            "該日期已過 → 查日報／官方後續，依 .claude/reporter-rules/shared.md「事實更正必回掃」處理：改掉＋回掃（先 `wiki_graph.py explain <頁> --section`，再拿關鍵字 grep 補漏）",
            "未過期 → 驗敘述與最新日報一致即結案",
        ],
    },
]
...
def main(argv=None) -> int:
    ...
    if args.cmd == "all":
        print(f"質疑題庫共 {len(BANK)} 題（單一來源；機制說明見 .claude/skills/wiki-lint-reader-acceptance/references/inquiry.md）")
        for q in BANK:
            _print_question(q)
        return 0
