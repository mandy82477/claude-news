  ...
  if (!header) {
    ...
    async function render() {
      ...
      const md = [
        ...
        "|" + header.map(() => " --- ").join("|") + "|",
        ...view.map(
          (r) => "| " + r.map((c, i) => cut(c, header[i]).replace(/\|/g, "\\|")).join(" | ") + " |"
        ),
      ].join("\n");

      // Obsidian 版本間 MarkdownRenderer API 名稱有變，兩種都試
      if (typeof MarkdownRenderer.render === "function") {
        await MarkdownRenderer.render(app, md, results, page, dv.component);
      } else {
        await MarkdownRenderer.renderMarkdown(md, results, page, dv.component);
      }
