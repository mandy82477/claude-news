...
> **最新功能缺口**（2026-06-18）
> Claude Code Artifacts 部分填補「平台可及性」缺口，工作階段進度可輸出為可共享互動網頁；但輸入操作（行動裝置或瀏覽器指派任務）仍無解，矩陣狀態從 ❌ 更新為 ⚡ 部分對應。
...
然而，此功能只覆蓋**輸出共享**，不涵蓋**輸入操作**（行動裝置或非工程師使用者仍無法直接在瀏覽器指派任務給 Claude Code）。社群對「在手機或瀏覽器操作 Claude Code」的需求仍未被滿足。
