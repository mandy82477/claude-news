...
官方 agent 積木在 2026 年中補上了最後一塊：dynamic workflows 讓編排從 Claude 的逐回合判斷變成一支可重跑的 script，先前「傳訊原語不含依賴排序」的編排缺口已被補上大半。本頁回答兩個問題：這件事該交給哪種 agent 形態（選型表），以及積木之間怎麼組（架構節）。Managed Agents 平台產品本身的現況、計費與零件成熟度在子頁 [[entities/managed-agents]]。
...
**三、把編排寫成程式。** 官方積木是 **dynamic workflows**（全部付費方案，[官方文件](https://code.claude.com/docs/en/workflows)，2026-09-10 查證）：計畫不再由 Claude 逐回合決定，而是由一支 JavaScript script 持有——`agent()` 派一個、`pipeline()` 逐項流水、`parallel()` 齊發等全收，中間結果住 script 變數，主對話只拿到最終答案。互動模式是**依 script 排定的依賴順序流動**：上游 agent 的輸出直接成為下游 agent 的輸入，還能讓獨立 agent 對抗式互審彼此的發現再回報。判斷式：**這個編排下週還會照原樣再跑一次嗎？** 會——寫成 workflow 存起來；不會——讓 Claude 臨場派 subagent 就好。社群拿它玩出什麼配置，本庫尚無收錄回報。
