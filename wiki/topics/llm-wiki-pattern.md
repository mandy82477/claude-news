...
- **查詢是第一級動作，答案回流進頁面**：`/wiki-query` 依問題型態走六路（Grep／index／標頭欄位／log／負責表／引用圖），答案附出處與「現在／當時」；wiki 沒有的事實查一手來源後寫回該頁並標查證日、[[log]] 記 Query 條目（2026-09-12 補上，對應 Ghelbur 的 31 個指令、Fulkerson 的指令路由）。
