...
> **最新成本管控動態**（2026-07-23）
> Business Insider 報導 Amazon 逐步降低旗下語音助理 Alexa 對 Anthropic 高成本模型的依賴，藉此削減 AI 相關支出——是繼 Lindy（100% 切至 DeepSeek，06-29）之後另一個具名大型企業因成本考量縮減 Anthropic 模型採用的案例，惟措辭為「逐步降低（weaned off）」而非完全切換，且僅標題可用，具體降幅、替代模型、時間軸未見細節（詳見「企業案例」新增段落）。07-13 已記錄之 1660 萬美元帳務錯誤等既有事件仍為本頁近期重大事件，未見進一步後續報導。
