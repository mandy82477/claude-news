...
查快照表看到「誰贏」後，這裡回答「贏在什麼題目上」。三種計分機制先認識：**盲測 Elo**（人類看兩個匿名模型的產出投票，勝負累積成棋類等級分，分數是相對的）、**任務通過率**（固定題庫，過就得分，%）、**錯誤率**（越低越好）。

| 任務 | 比什麼題目 | 分數怎麼來 |
|---|---|---|
| 寫 code（SWE-bench） | 真實 GitHub 專案的 issue：給模型整個 repo，要它寫出修復 patch | 跑該專案的測試，通過才算解決；分數＝解決率 %（Verified 為人工複核過的題目子集） |
| 寫 code（Aider） | 多語言（Python/Rust/Go 等）程式練習題，難度偏高 | 產出程式跑測試，限次數內通過算對；% 正確率，同時記每題花費成本 |
| 寫文案／聊天（LMArena） | 真實使用者當下輸入的任何問題（不設題庫） | 同題丟給兩個匿名模型，使用者投票哪個回得好；盲測 Elo |
| 查資料（Search Arena） | 需要上網查證的問題（新聞、事實、時效資訊） | 同上盲測投票，比查得準不準、引得對不對；Elo |
| 做網頁（WebDev Arena） | 同一個網頁需求（做個計算機、landing page…） | 兩個模型各自生出可執行網頁並排渲染，投票哪個更好用好看；Elo |
| 畫圖 | 同一段文字描述（人物、場景、藝術風格、圖中文字等各類 prompt） | 兩張匿名生成圖並排，投票哪張更符合描述、更好看；Elo |
| 生成影片 | 同一段文字描述生成短片 | 兩段匿名影片並排投票（動作合理性、畫面品質）；Elo |
| 語音合成（TTS） | 同一段文字唸出來 | 聽兩段匿名語音投票哪個更自然；Elo |
| 語音轉文字（STT） | 標準語音資料集逐字稿 | 字錯率 WER：轉錯／漏／多的字佔比，**越低越好**；另計速度與價格 |
| 電腦操作（Terminal-Bench） | 在真實終端機沙盒完成任務（編譯、除錯、資料處理、系統管理） | agent 自主操作到完成，驗收腳本判定；任務成功率 % |
| 文件解析（OmniDocBench） | 各類真實 PDF（論文、表格、公式、掃描件）轉結構化文字 | 對照人工標註答案算還原準確度（文字、表格、公式、閱讀順序分項）；綜合分 0–1 |
| 音樂生成 | 同一段風格描述生成音樂 | 盲聽兩段投票；Elo |
| Embedding（MTEB） | 檢索、分類、聚類、語意相似度等多類任務題庫 | 各任務標準指標（如檢索命中率）取平均；綜合分 |
| 情商（EQ-Bench） | 有張力的多輪對話（衝突、安慰、談判場景） | 模型判讀角色情緒／扮演調解者，與參考答案或評審模型比對計分 |
| agent 自主時長（METR） | 一批人類完成時間已知（幾分鐘到數十小時）的真實工程任務 | 找出「模型成功率恰為 50%」的任務時長——能撐越久越強；單位是分鐘／小時 |
| 幻覺率（Vectara） | 給一批短文請模型摘要 | 偵測模型量摘要中「原文沒有的內容」佔比，**越低越好** |
| 實際用量（OpenRouter） | 不是評測——OpenRouter 平台上各模型被真實呼叫的 token 量 | 用量占比排名；反映「大家用錢投票」，與能力無直接關係 |

> 讀榜提醒：Elo 類分數只在**同一個榜內**可比（LMArena 的 1349 和 WebDev 的 1705 不能互比）；通過率類要留意題庫版本（SWE-bench Verified ≠ Pro）；所有榜都測不到「跟你的實際工作流合不合」，最終仍以自己的任務實測為準。
