...
> **最新進展**（2026-08-07）
> Context / Token 管理線新增一則固定成本量化實測：headless 呼叫（`claude -p`）未加 `--bare` 時冷啟動約耗 15 萬 token，補上「多 agent pipeline 大量發起 headless 呼叫」這個場景此前未被量化的成本來源。Codebase 索引與記憶線新增一則失敗案例類型：agent 因「已否決方案」未被結構化索引，重新實作團隊已經殺掉的做法——為既有「repo 才是記憶體」框架補上「記什麼不該再做」這個子類別。
