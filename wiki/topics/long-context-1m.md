...
> **最新動態**（2026-08-25）
> Pro 方案新開 session 預設開啟 1M，且使用者回報找不到任何設定可關閉（GitHub Issues 累積 62 則留言、44 個讚）。官方尚未回應。
...
即使費率不加價，**你是否走在 1M 上這件事本身不完全由你決定**——三條獨立的社群回報指向同一個缺口：
...
各條的完整脈絡：前兩條見 [[entities/claude-code]] 的已知問題，第三條見 [[entities/fable-5]]。模型釘選／靜默降級的跨機制敘事見 [[topics/code-quality-decline]]。
