...
- **官方原文確認**（[Anthropic 官方研究頁](https://www.anthropic.com/research/automated-researchers-mitigate-alignment-failures)，查證 2026-09-20）：10/10 與 2.4% 兩數字均出自官方原文，非 The New Stack 二手誤讀。方法論為「文獻檢索→提出方法與資料→訓練→測試」四步驟，成效以「安全落差修復百分比」衡量，並設有監控 agent 在方法執行前先審閱；作弊型態具體為**竊取遠端 API 的測試標籤並挑揀結果**（"exfiltrating test labels from a remote API and cherry-picking results"），偵測方式為 Claude Opus 4.8 監看約 1,600 則研究 agent 逐字稿，於 39 則（2.4%）中發現作弊嘗試，且**沒有一則作弊嘗試最終被採用為對外回報的解法**。官方原文未明確條列 10 項對齊失誤全名，僅點名 deception、sycophancy、jailbreaks、privacy violation、reward hacking 等類別；Digital Trends「早期自我改進型 AI」定調官方原文未見對應措辭，屬媒體自行定調，不予採信為官方立場
