...
- **與 thereallo.dev 條目的關係**（查證 2026-09-20）：多方報導將技術發現原始歸功於 **thereallo.dev**（於 Claude Code 二進位檔發現隱寫指紋機制，自 v2.1.91 起即存在）
  - 查證 adioof 原文後未見任何引用或署名標示發現者，屬對同一發現的**二手評論／框架文章**、非獨立平行發現；惟原文未附引用，無法百分之百排除獨立發現的可能性
...
- **證據狀態**：報導始終未附具體技術細節、程式碼樣本或 CVE 編號；媒體確認的是「禁令存在＋替代工具」，不是「後門指控成立」
- **查證結果**（查證 2026-09-20）：Alibaba 禁用事件確認與下方 07-08～07-10「後門」爭議**為同一起技術發現觸發**（Tom's Hardware：Alibaba 係在「疑似隱藏的中國偵測後門」被揭露後禁用）
  - Anthropic 工程師 Thariq Shihipar 已就該機制公開說明：2026 年 3 月上線的實驗性反濫用機制，目的是防堵未授權轉售商與模型蒸餾，非針對中國使用者的監控後門
  - 惟 Alibaba 與 Anthropic 均拒絕就「禁令本身」個別置評——技術機制有官方說明，企業對企業的禁用決策仍無正式聲明
...
- **官方首度否認**：Anthropic 今日**首度公開反駁**中國官方「後門」指控本身——為整條敘事線（社群指控 → 官方警示 → 07-07「實驗」定調）中，官方第一次針對「中國官方層級」指控正面回應
  - **查證結果**（查證 2026-09-20）：反駁核心是中國使用者本就未獲授權存取 Claude／Claude Code，故警示中「須解除安裝」者本屬未授權存取；技術機制見上方 Q-02（2026-03 上線的反濫用實驗）
  - 公開檢索未見「Adnane Khan」與本事件的關聯，探針詞可能誤植，本頁不採信此人涉入
- **同日聲明**：Anthropic Blog 發布「Inviting hard questions」（07-10 12:19 UTC），正面回應外界對 AI 安全性、是否取代工作等疑慮的質疑；發布時間與後門否認相近
  - **查證結果**（查證 2026-09-20）：確認為不相關的獨立品牌／研究倡議，非同一波公關回應——與 Mother London 合製，聚焦工作機會、安全、家庭等廣泛公眾疑問
  - 官方文宣顯示活動實際啟動於 07-09（早於後門否認一天），主題與中國後門爭議無關，屬時間相近的巧合
...
| Alibaba 禁用 Claude Code（改用 Qoder）+ Meta 限制工程師使用 Claude | 🟡 觸發原因已確認為下列「隱寫術」機制被揭露，技術面 Anthropic 已說明⟨Q-02⟩ | 2026-07-06／07-07 |
...
**懸置細節**
- ⟨Q-02⟩ **查證結果**（查證 2026-09-20）：Alibaba 禁用 Claude Code 係因「隱藏中國偵測機制」被揭露觸發（見上方 07-06 條目）
  - Anthropic 工程師 Thariq Shihipar 已就該機制公開說明為 2026-03 上線的反濫用實驗；惟兩家均拒絕就「禁令決策本身」個別置評，Meta 限制使用一事始終無回應
