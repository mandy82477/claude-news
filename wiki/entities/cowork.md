...
**類型：** product
**狀態：** active（桌面版正式發布；行動版／網頁版 Preview，僅 Max 訂閱戶起步擴大中）
**領域：** 🛠️ 工具/功能
...
> **最新動態**（2026-09-12）
> Windows 九月累積更新讓 device_bash 於 ARM64／x64 皆失效（#92958），與 09-08 更新致 Plan9 掛載失敗（#92984）同源；官方尚未修復。

---

## 現況

Cowork 是 Anthropic 的圖形化協作介面，讓使用者指派任務給 Claude 在獨立 VM（沙箱）中背景執行，闔上裝置或關閉筆電後任務仍在雲端持續進行；桌面版已正式發布，行動版與網頁版自 2026-07-07 起以 Preview 形式開放，首波僅限 Max 訂閱戶（詳見 [[feature-radar]]「Claude Cowork 行動版 / 網頁版擴展」）。Windows 是這條產品線目前最不穩定的平台——本庫累積至少五起獨立的 VM／服務啟動失敗事件與多起資料完整性問題，完整清單見 [[entities/claude-code#已知問題]]，本頁不重複列出；一句結論：**Windows 上跑 Cowork，先假設它會出狀況，重要工作別只信任它的自動保存。**

**跟 Claude Code 差在哪：** Claude Code 是終端機 CLI，面向工程師逐指令互動；Cowork 是圖形化桌面／行動／網頁介面，任務丟出去後可背景執行，不需要盯著終端機。兩者共用底層 Claude agent 能力（如 2026-05-03 macOS 電腦使用功能，Claude Code 與 Cowork 同步取得直接控制桌面滑鼠鍵盤的能力）。2026-05-11 一則 Reddit 貼文（單一使用者觀點，非官方聲明）指出，桌面版 Claude Code 推出後與 Cowork 功能高度重疊，使用者一度分不清楚兩者定位；此後未見官方公開發文說明差異。
...
- 競品對標動態（Cursor「Sand」等）：[[topics/competitor-landscape]]
