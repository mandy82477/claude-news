...
| | Anthropic 開放 Claude Plugins 目錄自助提交入口，開發者可自行送件上架（Unite.AI 報導，僅標題可用，送審流程與審核標準未載） | 2026-09-25 |
...
- **The New Stack 報導的 20 萬 token skill 已確認並修復**（查證日 2026-09-20）
  - 該 skill 是協助開發者串接 Anthropic API 的官方 skill，載入一次耗費逾 20 萬 token。
  - 這與本頁「漸進式揭露三層架構」的設計初衷相牴觸——**屬機制未生效，不是理論缺陷**。
  - 已於 v2.1.234（2026-08-17）改為隨需載入參考文件，初始 context 成本降到約 25,000 token（降幅逾 85.7%）。
...
| 日期 | 事件 |
|------|------|
| 2026-09-26 | Anthropic 開放 Claude Plugins 目錄自助提交入口，開發者可自行送件上架（Unite.AI，僅標題可用） |
...
**懸置細節**
- ⟨Q-01⟩ The New Stack 報導某 Claude Code skill 在回答問題前耗掉 20 萬 token
  - 查證確認為協助串接 Anthropic API 的**官方 skill**（[The New Stack](https://thenewstack.io/claude-code-token-reduction/)，查證日 2026-09-20）。
  - 已於 v2.1.234（2026-08-17）改為隨需載入參考文件，初始成本降到約 25,000 token（降幅逾 85.7%），問題已解決。
