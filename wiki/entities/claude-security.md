...
**懸置細節**
- ⟨Q-01⟩ ❓ **待查證**（標 2026-08-10｜查 Workflow tool、存取權限｜訊 2026-08-10）：Reddit 使用者反映 Claude Security 沒有 Workflow tool 的存取權限；同日 [[entities/claude-code]] 查證官方文件確認 Workflow 功能需在 `/config` 內另行啟用「dynamic workflows」才可使用（opt-in 設計），可能即為該回報者的成因，但未見官方直接回應此則個案，是否為既有設計限制或待補功能嚴格而言仍未 100% 確認，維持待查證。
