...
## 2026-09-07 全站頁面 review 第 12 波：topics/code-quality-decline（廣度優先第 1 層第 8 頁；本波後暫停）

**Query（使用者）：** 「目前改了幾頁了」→「先改到這樣就好了 你覺得這幾波改動 cp 高嗎」→「還在跑就繼續跑 跑完這一波就停」。**點出什麼**：(1) CP 評估——第一輪掃樞紐頁每波都抓到讀者會被騙的事實錯誤（90%、靜默降級、Legacy 退役日、待彙整、三修版本、來源等級升級），但邊際下降，且值錢的是主編官方查證與冷讀者、貴的是設計者／評審來回（🔴 多為機械層）；接續建議樞紐全流程、中層葉子改「查證＋冷讀者＋Sonnet 直修」約五分之一成本。(2) 本頁核心論證建在已證偽宣稱上——「effort 非單調」五處寫成官方確認，opus-5 第 11 波定稿已判證偽；健檢卡評全頁品質最高的節，核心形容詞「靜默」是過期現在式（06-11 官方改可見）——查證要在健檢卡之前給它。

**主編官方查證：** 04-23 postmortem 三修各版本（effort v2.1.116、thinking 清除 v2.1.101、prompt 長度 04-20 revert 3%），「工程疏失」是媒體措辭；#41930 CLOSED 04-24、#65687 OPEN、#77136 OPEN 118 則 09-03 更新、#83510 OPEN 可重現量測；靜默 fallback 06-11 Wired 認錯改可見；09-03 狀態頁事件同日 16:16 UTC 解決；effort 非單調官方無確認。

**健檢卡校準（第十次）：** 卡點 4/5、跳數 0/5（量尺：健檢卡從本頁起算、冷讀者 index 算一跳）；自洽掃描漏同頁日期與計數、漏鄰居否定句反證；讀者語言閘零命中但冷讀者列十條——閘擋詞不擋口吻；分群切法：冷讀者「答案的家該在哪」優於健檢卡「變動節奏」。

**新判準首次產生拆子頁候選**：「模型釘選／靜默降級」三題全過、健檢卡與冷讀者獨立一致——使用者未回，設計者與評審皆判維持不拆（子頁約 30 行且 08-04 後無事件，10-03 撞併回門檻；回掃四處跨維護者；可見度已由摘要升位解掉；「行數低於鬧鐘」理由被評審駁回）。

**設計與評審：** 評審 7 🔴——則數 6／4／3 算錯（正 6／3／3）、導言「沒有一則帶測試方法」被自己表推翻、判準條文寫進正文四處（第 6 波立法後復發）、lint 步驟無觸發邊且 `gh issue view` 缺 `-R`、分工單邊、discussions:325 舊節名、規則檔指不存在的節；設計者 17 條全處理。

**成果：** 「三條線現在到哪」表、「你釘不住你選的模型」升摘要、「怎麼自己量一次」三步、三表判準、Opus 5 表壓 5 列、archive 12 則、規則檔＋lint 5m、帳本 8 筆、index 首屏「我沒升版但它好像變慢了」路由。

## 2026-09-07 全站頁面 review 第 11 波：entities/opus-5（廣度優先第 1 層第 7 頁）

**Query（使用者）：** 「好 那你照著這個繼續處理接下來的頁面」（沿用新拆頁判準）。**點出什麼**：本頁結論層落後自己的證據層 30 天——L52「定價定位待彙整」與同頁 L160「2026-08-08 官方查證 $5/$25」並存，且被 index 鉤子照抄六週；Team 方案讀者全庫無人對它說話（pricing 只寫「同 Pro／同 Max」）；opus-4-8 官方已 Legacy 而全站零處寫退役日。

**主編官方查證：** 官方發布日 07-24（頁面用收錄日 07-25）；Claude Code 整體預設仍 Sonnet、Opus 5 為「預設 Opus」且在 Max／Team premium／Enterprise PAYG／API 為預設（W30）；4.8 Legacy 至 2027-05-28、同價、知識舊 4 個月；升 5 兩個 breaking change（thinking 預設開、只能 effort high 以下關）；官方四基準（Frontier-Bench、CursorBench 0.5%、ARC-AGI 3、OSWorld）；三筆逾期懸置全結案——SitePoint 評測文 context／牌價／發布年三項與官方全錯，判來源不可靠。

**健檢卡校準（第九次）：** 卡點 4/5、跳數 1/5（根因：假設讀者不開本頁）；「答的是不是讀者那一種帳」第四次命中且首次落在表級（Team 維度）；**「沒有結論」不等於缺陷**——頁面誠實標示弱訊號是給讀者的結論，健檢卡判反；冷讀者行號口徑三頁少 24。

**設計與評審：** 推薦案兩張表取代十段散文；評審 7 🔴——導言「四個數字全量在官方自有的尺上」事實錯（CursorBench／ARC-AGI／OSWorld 是第三方公開基準）、「Pro 最強模型」全庫 10 副本漏三、相關議題靜默增刪復發（第 10 波同型）；節名採評審「這些數字是誰量的」（對照式節名會被讀成勝負）；設計者 17 條全採納；複核：懸置基線 140 是對的（rebase 帶進 2 筆、閘只擋減少不擋增加）。

**成果：** 「你現在拿到的是什麼」兩代七列（4.8 退役日首次全站落地）、「這些數字是誰量的」、三處過期宣稱砍、三筆懸置結案、opus-4-8 Legacy 更正、規則檔「模型頁兩代對照表」泛化＋「上線天數不硬編」、lint 5l 擴充、index／radar／overview 七處、帳本 H-fa9b87。

## 2026-09-07 全站頁面 review 第 10 波：entities/fable-5（廣度優先第 1 層第 6 頁）

**Query（使用者）：** 「只有 archive 沒有新的主題喔」→「你覺得什麼時候該開新頁」→「有一個頁面回答太多問題或主題也可以拆，就是子頁面機制」→「行數太多是不是也可以考慮拆」→「母頁也可以變索引頁跟重要結論？」→「我沒有說只能拆 log」→「要不要拆子頁要看是不是這一頁回答的問題已經太多了」→「好 那你照著這個繼續處理」。**點出什麼**：九波 13 頁定稿零新主題頁，不是資料沒有，是拆分判準按節列候選、且「一個故事裡長出另一個故事」靠感覺；使用者改為**按問題數判**——考題分群、每群一子頁、母頁留索引＋重要結論，不按行數／月份／層切（主 session 先提的「證據簿」切法被否決）。已寫進 `wiki-ingest-format.md` 頁面拆分原則（09-07 版）與 skill 原則 3。同日 skill 由 8 檔 17,781 字改單檔 4,333 字（使用者：只留原則／角色／派工／邊界）。

**新判準首次套用**：fable-5 考題五群（模型本身／出口管制／配額過渡／護欄／費馬事件），拆子頁候選 0——存取價格住 pricing、管制住 gov-policy、選型住 model-comparison，本頁留的是舊副本；冷讀者「拆三頁」與健檢卡「歸檔」九成同一結論，按新判準管制段是封存不是獨立頁。

**主編官方查證：** Fable 5 官方已標 Legacy、退役不早於 2027-06-09（全站零處寫）；5.1 同價、cache read $0.25（5 為 $1）、知識截止 Jun 2026、官方推薦「demanding reasoning and long-horizon agentic work」；**「護欄會靜默降級」與官方相反**——〈Redeploying Claude Fable 5〉「Users will be notified… sent to Opus 4.8」，support 有專文，四個觸發類別；09-04 費馬事件用「約略相當 5.1 的內部研究模型」非 Fable 5；Max 50% 週配額／Pro credits 是常態分流非促銷。

**健檢卡校準（第八次）：** 卡點 3/5、跳數 3/4。新盲區：過期「未明／待觀察」型宣稱是第三種互斥（radar「Max/Team 動向未明」vs pricing 已查證）；同一數字跨頁被升級來源等級（本頁自承 Reddit 轉載的 46%／96%，選型頁與 patterns 寫成官方基準，第 8 波 verified 也沿用）；預測跳數要問落在哪張表。

**設計與評審：** 評審 6 🔴——熱度表規則檔 h3 過不了 `table_census`（設計者拿手打簡寫測的不是稿子）、爭議節自訂上限 10 落地 12、相關議題靜默刪 ai-agent-safety、帳本抄舊行號、懸置基線算術（短標記不計指紋）、三表零週期看守→lint 5l；回掃射程評審判 model-comparison 三處本波改（同維護者事實更正不算跨頁重寫），採納；設計者反駁 1 成立（opus-4-8:46 確有管制句）。實作單 10 步，複核放行 0 🔴。

**成果：** 412→約 316 行；兩代對照七列固定；護欄節四類；archive 新建（管制立場表、配額過渡節、06 月）；回掃六處；規則檔三表判準＋C 條「同一數字多頁以最保守方為準」；index 首屏補「要不要換新旗艦」路由；radar Legacy。

**複驗：** Q1／Q2／Q3／Q5 ✅、Q4 ⚠️；退役日首次落地讓 Q2 由 3 跳半變 1 跳。殘留最後一批修：50% 常態 vs 促銷結束、護欄表世代限定、靜默兩義。

## 2026-09-06 全站頁面 review 第 9 波：topics/community-tech-discussions（廣度優先第 1 層第 5 頁）

**Query（使用者）：** 「好啊」（裁決砍「🔥 本週熱點」整節）、「好 砍」（再確認）。**點出什麼**：承諾「現在」的兩個節都不是現在——本週熱點十則八則 06 月、「最近 30 天」表有 05-05 列；結論五條無日期無指回路；「90% AI 生成程式碼漏洞」全庫查無出處；Skill Atrophy 寫成「社群共識」卻沒引 Anthropic 自己 01 月的隨機對照試驗。

**主編官方查證：** 80% 系統提示詞是 claude.com 07-24 官方發文（頁面 07-26 是 HN 討論日，官方一手被記成社群訊號）；thinking 文件 5 世代預設 `display: omitted` 且 Fable 拒絕誘出推理，比頁面「僅回摘要」更嚴；90% 外部研究為 45–62%（Veracode／OX）、安全發現 10 倍（CSA）；Anthropic RCT 17% 理解度下降、用法決定退化程度；auto mode 官方＝分類器審每個動作、對話邊界隨壓縮遺失。

**健檢卡校準（第七次）：** 卡點 3/5、跳數 2/5、Q5 判錯方向。**純散文互斥零覆蓋**：L47「形成共識」vs L105「尚無共識解法」同頁矛盾、L114「至今無後續」vs 人物頁兩則後續，健檢卡自洽掃描只掃帶數字或符號的句子，連兩波由冷讀者代打。

**設計與評審：** 推薦案「目前結論＋本週熱點併為『現在吵到哪』」；評審 7 🔴——最重的是**狀態三值不可重跑**（三個人算「最後一則證據」三個日期），最小改法：證據欄寫被引條目標題前八字，撈法變一次 grep，本波做完；設計者第一版第 3 列違反自己寫的 60 天線（06-24 距今 74 天）；複核抓到蒸餾三則正文與 archive 兩邊都在、表下指路段整段漏做。

**成果：** 「現在吵到哪」10 場（五還在吵／二共識／三僵住）帶最後證據日、證據硬度、官方怎麼說；熱門討論表到期線 54→46 列；05／06 共 90 則搬進新建 archive；規則檔三張表判準、lint 5k、registry 錨點更名；index 首屏補「社群在吵什麼觀念、哪些吵完了、哪些還僵著」路由列。

**複驗：** 五題全部 2 跳內拿到（Q4 半：結論拿到、日期兩處打架）；Q1 半→拿到、Q4 沒→拿到。殘留：90% 標題（免斷錨加註）、patterns vs trends 分不出（回訪）。

## 2026-09-06 全站頁面 review 第 8 波：topics/community-tech-patterns（廣度優先第 1 層第 4 頁）

**Query（使用者）：** 「我剛看了一下 我覺得 pricing 現在會不會蠻值得拆頁」→「之前沒拆是因為我沒說可以拆嗎？請問現在 skill 會不會過度複雜」→「不能原則就好嗎」→「重要的只有我說的原則、agent 角色設定、派工方式和邊界處理，其他的都很沒必要」→「好」；另「好啊」裁決 timeline 併入 archive。**點出什麼**：(1) 拆頁三題判**按節列候選**，看不見「訂閱者 vs API 開發者」這種按讀者的分岔——pricing 的 API 側（通路與乘數，5 個錨點入邊佔 3）從未進候選；死案（計費切割風波 102 行、0 入邊）判「凍結」而非歸檔。根因不是授權，是候選產生方式。處置：skill 原則加「拆頁候選按讀者類型列」，pricing 回訪改為歸檔切割風波＋API 側拆頁評估。(2) skill 五波內 1 檔→8 檔 17,781 字，每波校準把一次漏掉寫成一條掃描，清單越長 agent 越打勾而不思考，本波照七條掃完仍漏「缺口表 ❌ 對同頁證據」。處置：改單檔 4,080 字（原則 8 條／角色表／派工順序／邊界），六問細掃描、評審實例病清單、契約、模板全砍；加一條必先併一條，教訓只進 ledger 校準表。

**主編官方查證：** subagents／agent teams／cross-session 三份官方文件——官方預設是模型拆（「Claude automatically delegates」）、巢狀 ≤3 層、fork 繼承全脈絡、背景預設、SendMessage 名冊；agent teams 實驗性且 `TeamCreate` 已不存在；cross-session v2.1.224+ 純文字不代批權限；**orchestrator／worker 分模型已有一級支援**（subagent `model` 欄、`CLAUDE_CODE_SUBAGENT_MODEL`）——頁面缺口表寫 ❌ 未補而同頁 L71 引官方基準，自相矛盾。

**健檢卡校準（第六次）：** 卡點 3/4、跳數 2/4。冷讀者抓到健檢卡沒預測的頁內矛盾（狀態表 ❌ vs 同頁證據）；首屏「答的是不是讀者那一種帳」第二次命中（L31 送進工具貨架）；patterns／discussions 版型同構（1100 行＋技術彙整＋目前結論）冷讀者分不出，但分工是做法 vs 觀念，屬版型不屬併頁。60 天沉寂候選三方三數（健檢卡引 log 7、設計者 3、評審重算 4）——「最後動態」定義不寫死就沒有可重跑的答案。

**設計與評審：** 推薦案「頁不拆節不砍證據不動，只重做 120 行結論層」；評審 4 🔴（短標記不合 `SHORT_RE`、規則檔 h3 無 slug 普查不亮、去向表錯行讓「分模型翻已補」落錯列、timeline 殼 index 未同批）12 🟡；裁「併」後評審補 2 🔴（引用全庫 7 處非 3、標題層級不能直接 append）；設計者兩輪 15 接受 4 改寫 0 反駁；實作 11 步，複核放行 0 🔴、21 列全核、併頁 264 行一字不缺。

**成果：** 概覽表 25→21 列覆寫式＋最後動態欄（單一口徑＋兩段式撈法＋60 天退場＋補位＋入口）；目前結論改指得回節點；學術對照補官方三層階梯與資料截至；缺口追蹤三值文字狀態；誰負責拆分五列固定加官方列；懸置 6→7 短標記化（基線 142）；規則檔「四張結論表」8 條＋主線 tag 補填＋鉤子失效轉知；lint 5k；**timeline 併入 patterns-archive 留殼**、7 處引用改指、母頁 6→7；index 補「社群現在怎麼玩」路由、鉤子改 21 類盤點、tools 鉤子去工具數。

**複驗：** Q2／Q3／Q4 半→2 跳拿到；Q1 仍半（「跟兩個月前差在哪」要到 trends 拼演進列）。殘留：首屏「我卡住了」列仍在前、「主線：—」外洩、四頁四 callout 日期——列 09-13 回訪。

## 2026-09-06 全站頁面 review 第 7 波：topics/anthropic-business（廣度優先第 1 層第 3 頁）

**Query（使用者）：** 無新 Query（沿用「接續著前面幾波的節點繼續往下做」授權）。**點出什麼**：本頁四個核心數字（估值、ARR、採用率、補貼倍數）各有 2–4 個版本分住不同節或不同頁，沒有一處回答「現在是哪個」；「17 倍訂閱補貼」是 token-xray 05-28 單一社群計算，被抬成 index 鉤子與風險表事實。

**主編官方查證：** 估值三版本定位（3,500 億＝04 月 Google 輪歷史、9,650 億＝05 月 Series H 最後定價、2 兆＝FT 08-13 投資人預期非公司目標→⟨Q-05⟩ 結案）；Ramp 8 月指數 43.5% vs 39.7%（頁面停在 05 月 34.4%）；ARR 650 億（7 月）一致；S-1 06-01／02 機密遞交皆二手、預期 9–10 月上市；17 倍外部零獨立佐證。

**健檢卡校準（第五次）：** 卡點 2/4、跳數 0/4（四題全低估一跳）。Q4 失誤根因：把首屏射程當連通性問題，其實是**適配性**——pricing 通路表答 API 價格，讀者四萬美元大半是訂閱席次，答得了價格答的是另一種價格。另兩盲區：反證住鄰居頁兩頁互不指認；「該合併」只用機器分數與分工句，冷讀者分不分得出是第三證據。三項已入 skill。

**設計與評審：** 四張表全改覆寫式結論表；評審 7 🔴 14 🟡——抓到補貼「13–44」把月費暴增倍數混進補貼倍數（正解 13–40）、S-1 選邊、合作節 86→6 行數反增且一行制無界、pricing 通路數字抄成第二家、時序方括號條文上線即讓存量違規；設計者第二輪 20 接受 1 反駁（L330 確有 ⟨Q-02⟩，評審漏看）。實作複核不放行 2 🔴：**保命條款第四次命中**（兩筆 🔎 標記被溶成散文）、**條文洩漏復發**（讀者語言閘不含「蒸餾／封存／保留最近」）。

**成果：** 928→797 行；「現在的數字」10 列（來源等級五值＋下一個時點＋圖例）、IPO 五格表、風險 6 列、合作衝擊 6 列＋一行制留最近 3 月；archive 新建；規則檔 4 行→11 條、lint 5j；跨頁回掃五處；帳本 6 筆（`pending_handoffs.py` 補「投資分析」「開發實務」類別）；index 首屏補商業路、鉤子去 17 倍。順帶修四件機械假象：`wiki_graph explain` 行號改檔案原始行號、時序邊節名歸屬、`table_census` 無日期欄回 unknown、機制命中須含頁名（12 新測試，後改合成 fixture）。

**複驗：** **四題全拿到**（Q1 2 跳「五種數字不可互相替代」解掉困惑；Q2 頁面自己標明 34.4% 是歷史；Q4 答成「都不會、真正咬人的是 Bedrock 拿不到 Batch 折扣」）；殘留＝跨頁數字打架（ECM「40–44」、market-signals 三處描述本頁錯誤走帳本）。

**教訓：** (1) 保命條款人工提醒四波四次失效，要機械化——懸置標記總數不得靜默減少（ledger 機制待辦）；(2) 讀者語言閘缺規則條文詞，設計者逐字稿即使分三欄仍會漏一段——閘要補詞；(3) 主 session 加測試時不得硬編真實頁面節名（自己剛加的 4 個測試被本波改版打紅）；(4) 廣度優先下一頁是別的 session 在改的頁時，唯讀階段照跑、實作前確認對方收尾，commit 訊息註明混入。

**待辦：** 回訪 09-13（ledger）；併四人物頁裁決懸；下一頁 community-tech-patterns。

## 2026-09-06 全站頁面 review 第 6 波：topics/anthropic-government-policy（廣度優先第 1 層第 2 頁；併三人物頁）

**Query（使用者）：** 「好」（裁決：併 chris-olah／tom-brown／chris-ciauri 三頁；ai-agent-safety「目前結論」整節砍）；同日「妳整體整理一下這個 SKILL 吧～會用到的相關 SKILL 規則應該都同屬於同一個 SKILL FOLDER」「請清理」→ skill 重整為 SKILL.md（入口＋執行順序表）＋dispatch-prefix.md（前綴與契約）＋templates/ 六份角色模板。**點出什麼**：四波以來派工 prompt 每波臨場寫，模板化後前綴逐字相同才有 prompt cache 與可比性；skill 引用的文件散在 docs/ 底下多一跳。

**主編官方查證：** 香港高盛／OKX 被切成因＝Anthropic 區域政策不允許港／陸（Goldman 合約不涵蓋港；OKX 8 月初帳號停權後恢復、改導向他模型）——頁面「成因未明」結案且是全頁唯一直接打到讀者的事實；Sanders 法案實為 09-03 Sanders＋Casar「Ban Artificial Superintelligence Act」最高 20 年（頁面「數年徒刑、僅標題」）；五角大廈黑名單 08-27 裁定違法頁面已有，補 D.C. 巡迴第二案待判；Defense in Depth fallback 官方 06-30 原文（改送 Opus 4.8、會通知、只寫 Fable 5）；出口管制四日期一手定位：06-12 生效、06-30 商務部通知解除、07-01 恢復存取——設計者以頁面二手記錄主張「07-01 才是解除」被推翻，**一手優先、用頁面自己的記錄反駁一手是倒過來的**（已入 skill）。

**健檢卡校準（第四次）：** 卡點 2/4、跳數 1/4。方法論失誤：首屏路由只查了指向本頁的 index L109，沒查指向鄰居的 L107（中國頁停在 07-11 卻被 index 指為中國議題入口）；讀者比預測更早放棄往下鑽。人物頁判「缺一頁」否：Lutnick／Sanders／Hegseth／Helberg 在本頁已有具名記錄，開頁只會複製事實而無供料——冷讀者要的是「什麼在動、下一個時點、會不會咬到你」那張表，人名是欄裡的值。

**設計與評審：** 34 列變數表→8 列線表（判準：具名主體可查證動作或直接產品後果；「會不會改到你用的 Claude」與「下一個時點」欄不得問句；退場**分流**——標「會」的列不適用 90 天，否則 Defense in Depth 09-30 會被自己抹掉）；評審重算 34＝8＋9＋17 成立、三週攻防列零新線；併頁機械配套三處會紅（殼領域繼承、識別字「已併回」、投影）→ 主 session 決定 redirect 排除 index 投影（三腳本各一行，含 `test_index_sync` 豁免）；設計者第二輪反駁評審「四人白名單」改有定義域的款②、人物表 11→9 列；新增保命條款實測（SOFX 先補時序）。**實作複核不放行**：日期口徑五處未改、兩筆懸置無查證被刪（保命條款第三次命中）；複驗再抓 27 行判準條文寫在正文、8 處「詳見『## 目前局勢』」死引用——設計者 draft 把規則條文當頁面文字，讀者語言閘不含這些詞。已修。

**成果：** 8 列線表＋「政府動作對你的產品做了什麼」＋人物表 9 列；懸置 23→11、逾期 9→4；蒸餾 62 列進 archive（先修 archive L41 反向指針）；三人物頁併入留轉址殼、index 三列刪；規則檔 `wiki-ingest-safety-policy.md` 指向不存在節的斷鏈修好、退場分流、入口判準；`/wiki-lint` 5i 擴為兩頁；帳本 5 筆；ai-agent-safety 四條結論併入。

**複驗：** Q1／Q2／Q3 半→**拿到**（前 125 行「全站最好的設計」）；Q4 半（人物表連向轉址殼折返→改純文字）。

**教訓：** (1) 判準與退場條文的家是規則檔，頁面只留一句讀者語言——設計者逐字稿要分「進頁面」與「進規則檔」兩欄；(2) 併頁要配三處機械配套且測試要豁免殼；(3) 保命條款連三波命中，實作者對「搬家」「唯一的家」仍會讀成免動作——去向表每筆懸置寫動詞；(4) 首屏射程含鄰居列。

**待辦：** 回訪 09-13（ledger）；下一頁 anthropic-business。

## 2026-09-06 全站頁面 review 第 5 波：topics/ai-agent-safety（廣度優先第 1 層第 1 頁）

**Query（使用者）：** 「請從指頂節點開始 BREADTH-FIRST」→「不是我是說從已存在的 WIKI 頁面開始」→「剛剛那一套 review 機制，執行的方式要用 breadth-first，A 節點做完，接著做他的所有子頁面 review，再往下」→「就接續著前面幾波的節點繼續往下做」→「等等 我沒一次教你做五頁欸」。**點出什麼**：(1) 選頁順序改為從已審 8 頁往下 BFS（第 1 層 37 頁：15 樞紐／12 中層／10 葉子；`scratchpad/bfsq.py`），不再用三條讀者路徑挑；(2) 主 session 提「五頁一批」報價未獲批准即派 7 個 Opus agent，被叫停——**報價單存在不等於批准，「接續做」＝沿用原規模**（記憶已存）。同日另兩則 Query：「我以為設計者也考慮新增併砍留」→ skill Q4 加**節點三問**（存在必要／該延伸／該合併，使用者原話）；「skill 你可以再簡潔一下嗎／改版日期都可以刪掉／評審不用讀 REVIEW-PRINCIPLES」→ skill 68→62 行、派工前綴重寫、全域規則加例外。

**主編官方查證：** Anthropic 把 embracethered 的 Auto Mode 60–80% 注入揭露結案為 informative，定性「best-effort convenience control，不是安全邊界」——本頁最缺的一句；EFS 官方 09-01（頁面寫 09-02）、監看網攻／生物／憑證外洩非提示注入防禦、秋季分階段、免費；infostealer 官方處置＝強制登出、移除付款方式、退款，平台未入侵；89% 攔截率出處為官方 08-07 blog（Trajectory Labs 720 次注入 0%）。

**健檢卡校準（第三次）：** 卡點 1/2、跳數 2/2。真盲區：首屏 index L29 把讀者送到 claude-code「現在會咬到你的」表，該表無安全條目——**被送到錯的表比沒路更糟**，六問只問「有沒有路到本頁」；懸置 metadata 裸露 23 筆全過讀者語言閘。兩問已加進 skill。規則檔 `wiki-ingest-safety-policy.md` 叫記者 prepend 到不存在的 `## 事件記錄` 節，是三張成長表零退場的根因。

**設計與評審：** 設計者提「現在會打到你的」11 列（照 claude-code 骨架多一欄「官方擋到哪」）、防護對攻擊面一對一、「目前結論」82 列為第三份副本；評審 5 🔴 8 🟡 全附修法，抓到主 session 代判「10 列全收」算錯（合格 11）；設計者第二輪 13 條全接受，新增**保命條款**：砍任何列或節前先確認不是某懸置僅存的家（SOFX 命中→先補時序再砍）。實作複核再抓第二個命中：Benzinga 那筆被實作者讀成免動作而靜默刪掉——已補回。

**成果：** 1175→約 930 行；結論表 11 列每列寫「誰會遇到」「你能先做什麼」（複驗評為整頁最值錢的一欄）；EFS 歸位；懸置 21 逾期→8 事實、14 件定家、去重後 15 筆；蒸餾 2026-06 13 條進 archive；規則檔退場＋補位＋入口判準、`/wiki-lint` 5i；index 首屏補安全路由並堵住 L28 誤導路；claude-code 表下一句指本頁；帳本 4 筆。

**複驗：** Q1 ✅、Q2 半→**拿到**（三個動作分住表格三列）、Q4 ✅、Q3 半（EFS 日期跨頁不一，商業頁走帳本）。

**教訓：** (1) 派工規模不可自行放大；(2) 主 session 代判的數字也要進評審重算；(3) 「搬家」與「免動作」對 Sonnet 實作者是兩回事，去向表要寫動詞；(4) 讀者語言閘綠不等於合格，metadata 裸露要另計。

**待辦：** 回訪 09-13（ledger）；砍「目前結論」整節裁決仍懸；下一頁 anthropic-government-policy。

## 2026-09-06 全站頁面 review 第 4 波：feature-radar ＋ entities/claude-code（P2「該不該升版」；使用者授權自跑）

**Query（使用者）：** 「妳這個跑完就自己沿著路徑再跑個兩頁」→ 主 session 選 P2（P1、P3 已各有定稿頁），使命句自定、只在拆頁／砍整節時問。**點出什麼**：三處在記同一批版本（radar 升版風險、radar 月節、claude-code 最新版本 121 列），卻沒有一處回答讀者唯一的問題——「從我的版本升上去會遇到什麼」。

**流程：** 6 新 agent＋6 次續用（健檢卡對照、設計者兩輪、評審改版一次＋複核、實作者三批）。評審在設計者第二輪期間依主編新查證重寫了評審，設計者又對新版重做——**續用的代價是版本同步**：實作者一度拿到舊 draft（10 列）做完才對齊 12 列版，主 session 要負責把「哪份是現行」講清楚。

**主編官方查證（記者無 web 工具）：** (1) `gh api` 抓完整 CHANGELOG 6,358 行本機 grep——v2.1.212／215 逐字確認，2.1.220→261 **無 breaking change 但 17 條預設值改變**（2.1.232 非隊友 agent 生成預設改背景、2.1.233 Todo 工具在新模型拿掉、2.1.222 移除 ultraplan、2.1.257 預設 Fable 5.1、2.1.261 keybindingFlavor 失效）；(2) 品質退步官方 04-23 postmortem 明載 **v2.1.116 全數修復**，code-quality-decline 掛了 27 天的「是否已恢復」懸置結案；(3) claude-code 13 筆逾期懸置 7 結案（Cowork Chrome／mobile 官方、`/design` 官方 preview、Workbench 08-17 下線、Apps Gateway **事實更正**——官方文件有專頁非「第三方生態」、空 thinking block 為顯示行為非計費 bug）；(4) `gh issue view` 三個升版風險 🔴 仍 OPEN，#53940 標題即 Cowork——冷讀者判「以上皆非可直接升級」矛盾，實為頁面漏寫限定詞。

**健檢卡校準（第二次）：** 卡點 4/4、跳數 3/4（口徑統一後可信）；Q4 沒對上是「讀者起點」假設錯。兩個真盲區：結論句對同頁反例、同頁挑最大值（快取降價 75% vs 三說）。內部用語閘清單缺口連兩波命中。

**評審抓到最值錢的一條：** radar 正在推薦一個已廢掉的設定——`keybindingFlavor readline` 08 月推薦、2.1.261 失效，全覽表仍 ⚡ 有條件推薦＋附設定範例。**推薦沒有回訪機制**，這是第 4 波之後要立的規則。

**成果：** radar 874→862 行：「⭐ 現在值得跟的三件」每則附「怎麼開始」；「從你現在的版本升上去，會遇到什麼」12 列階梯表（判準：破壞性一律列、預設值改變只列寫得出誰會遇到的；退場線 3 個月）＋「與版本無關、已生效」段（auto 8/14）；熱度與試用價值只住全覽表（刪 39 處副本，`check_feature_radar.py` 加單一家檢查、驗紅驗綠）。claude-code 804→796 行：「現在會咬到你的」7 列（Cowork 靜默截斷／Linux OOM／Cowork VM／MCP 四條，每列寫誰會遇到與你能做什麼）；七筆結案寫成 09-06 彙總列；競品／實用工具併入相關議題。code-quality-decline「當前狀態」改寫＋動作句（換模型還是等：證據不足以支持換一個模型就好）。規則檔：`wiki-ingest-features.md` 熱度單一家、階梯表維護、主編抓 changelog 步驟；七處節名消費端同步（ingest 指令、index、overview、app.js／index.html、build_web.py、inquiry_bank.py）。

**複驗（原四題）：** Q1 半→**拿到 2 跳**（「依據是官方 changelog 逐版核對」被冷讀者評為全站最有說服力的一句）；Q2 半→**拿到**；Q4 拿到；Q3 仍半拿到——區塊名「這禮拜」含 07-25 條、Fable 5.1「怎麼開始」`/model` 與詳細條目「官方未給 model id」自打——已修（改「現在值得跟的三件」、補官方 id `claude-fable-5-1`）。複驗再抓：階梯表與版本表對 v2.1.233／234 內容不同（來源不同，補說明句）、日期 08-17 vs 08-18 兩處（欄名分「發布日／報導日」）。

**教訓：** (1) 設計者判準沒帶讀者身分就會手挑列——第二輪把軸從「場景寬窄」改「寫不寫得出誰會遇到」，12 列變成可推導；(2) Sonnet 實作者兩處自報偏離一可接受一不可（🧠 組頭統計改反），評審複核仍必要；(3) `check_cell_limits` 指紋含懸置括號內日期，只改複查日就脫離基線——設計缺口記待辦；(4) 版本階梯表的完整性靠「每週有人抓 changelog」，漏做不報錯，是本波唯一沒機械看守的單點，待寫 `scripts/changelog_ladder.py`。

**待辦：** 回訪 2026-09-13（ledger 第 4 波清單）；砍「近期重要更新（2026 Q2 精選）」裁決懸；他頁洩漏與 skill-interest-watch 星數矛盾記 ledger 回訪第 5 條；帳本 6 筆。

## 2026-09-06 全站頁面 review 第 3 波：entities/pricing ＋ topics/enterprise-cost-management（P3「成本與衝擊」；CLI 續用軌首用）

**Query（使用者）：** 「P3，熱度 (b) 不成法，空目錄刪掉，出報價單」→「還有建議哪個副用嗎」→「A」（使命句）→「妳這個跑完就自己沿著路徑再跑個兩頁」。**點出什麼**：成本線是使用者標記的網站重點領域，pricing 入邊 136 全庫最高卻從沒健檢過。

**流程（雙軌之 CLI 軌首用）：** 6 新 agent（健檢卡＋冷讀者平行、設計者、評審、Sonnet 實作、複驗冷讀者）＋ 4 次 SendMessage 續用（健檢卡做預測對照、設計者第二輪 14 條處置、評審實作複核、實作者兩批修正）。續用每次 3–8 萬 token，取代第 1 波 28.5 萬的設計第二輪全量重讀。中途額度耗盡，以 session cron 三小時後自動接手（跨 22 UTC 雲端班次，先 pull 再實作，pricing 行號 +9 由評審在抽驗時吸收）。

**健檢卡校準（首次）：** 卡點 4/4 命中、跳數 0/4（健檢卡把頁內捲動當一跳、冷讀者把開頁當一跳）。六問抓不到的四項記 ledger：首屏路由、數字自洽（L216「184 僅次於 67」）、鄰居清單要機械產生（憑印象漏了真被搞混的 pricing↔model-comparison）、讀者語言閘缺維運句。

**主編官方查證：** 頁面七項數字全對；**一處事實錯誤**——08-21「$35M 額度供企業導入 Mythos 5」實為 Defender Advantage Fund（給修補開源漏洞的組織），企業端是 Enterprise 可用 Claude Security × Mythos 5、標準 token 計費；促銷說明頁一手原文（09-13 23:59 PT、5 小時窗不受影響、該頁不提 09-14 +25%——那是 08-29 Bluesky）；Spend Controls 官方 blog 07-02 三層粒度，ECM 三項 🧪 結案；官方不公布配額絕對量、無 Fable 5.1 每小時算例（頁面以官方 Opus 5 算例 token 量代入牌價，標「推算」）。四筆逾期懸置與雲端 09-05 新加的一筆全結案。

**裁決：** 使用者選使命句 A；主 session 代判 3 裁決點（砍「費用管控技巧」、砍 ECM「目前結論」、讀者語言閘加四詞）與 4 條事故狀態；設計者反駁評審一條成立（「已掃日報至」是 wiki-lint L94 指定措辭，加禁詞會互打 → 改「四處同改」待辦）。

**成果：** pricing 753→688 行：三張結論表打頭（我的方案／一小時多少／事故還在不在）、事件流退成證據層；22 條事故五值狀態（🔴 ✅ ⛔ ⚖️ ⏸）、總表 6 列＋留表優先序寫進 `wiki-ingest-commercial.md`；蒸餾 2 時段建 `entities/pricing-archive`（全庫第一個 entities archive，`check_hierarchy` 綠）；砍 4 節。ECM 缺口表結案且明示 Enterprise-only、指標表重算（案例 12）、砍「目前結論」＋`%%` 防 lint 長回。index 首屏補「這個月我會多花／少花多少」路由。讀者語言閘 26 詞、基線 25 筆／8 頁。

**複驗（原四題）：** Q2 半拿到→**拿到**（$1.33／hr、$117／月，逐筆驗算自洽）、Q3 1 跳半拿到→**拿到**（總表為「全站最好的交付」）；Q1／Q4 由拿到改半拿到——**不是退步，是頁面改成不虛報**（官方不公布絕對量、Spend Controls 非 Enterprise 拿不到）。複驗再抓 ECM「三個獨立方法論指向 40–44 倍」撐不起（第三個來源無倍數）＋ 10 條洩漏（「本表上限 6 列」「示範列」「本庫」「口徑」），已修。

**教訓：** (1) 實作者（Sonnet）判事故狀態時三條逾 90 天仍標 🔴、且把「表滿載不納入」當成做法——退場條文寫了三句對「滿了怎麼辦」給三個答案，評審複核才抓到；規則條文寫完要自己拿六列跑一遍。(2) 設計者第二輪的價值不在重寫，在「評審方向對、話沒說完」的四處逐字補完。(3) 評審 🟡 級的「示範列違反自己判準」在 ECM ✅／🧪 那格又出現一次——狀態值定義寫在表下，示範列就得照它填。

**待辦：** 回訪 2026-09-13（ledger）；model-comparison L99／L119 編輯政策洩漏（第 1 波頁面，記 09-12 回訪）；「已掃日報至」四處同改；ECM 🧰 圖例（社群格式）。
...
## 2026-09-09 Query：讀者標籤改設定檔＋節名翻譯腔一次改乾淨

**點出什麼：** 使用者問「開發實務有些是不是 TAG 錯」，接著三句話拆掉三層問題。

**① 成員資格是掃出來的，不是決定出來的。** `💻 開發實務` 的名單由 `build_web.py` 掃 `wiki/index.md`「開發實務入口」表的所有 wikilink 產生，**不分主次**——每次有人在某列補一個「見 X」出口，X 就悄悄入夥。實證：09-03 commit `8ed0f2bc` 專門把 `claude-code` 擋出 tab（改成只抽表格列、導言不算），09-06 第 4 波在表裡加了「我在舊版本，升上去會壞什麼」一列，它就從表格列這條路靜默回來，中間沒有任何裁決。另查出一個死 id：`feature-radar` 被抽進名單但它不是 KB 條目（走 `parse_radar`，不在 `entities+topics`），永遠拿不到 chip。

**處置：** 成員資格搬進 `data/reader-tags.json`（tag → 定義句 → 成員），index 入口表回歸單純路由。**關鍵是使用者裁決「標籤只給 HTML」**——標籤是呈現層決定不是頁面事實，所以不進 wiki 標頭：19 頁一行未改、六份記者規則與格式規則零改動，也不必處理子頁繼承／archive／記者誤刪三個風險。成員改動從此出現在 git diff 上。新測試 `test_reader_tags.py` 五條（死 id／缺定義句／前端 chip 沒跟上／已裁決不列者回流），雙向驗紅過。

**② `market-signals` 不算開發實務**（使用者：「market signal 不是程式」）——投資訊號判讀，不回答「怎麼做／裝什麼／選什麼」。移出標籤，但它在入口表的路由連結照留：**標籤與路由已脫鉤，這正是 tag 觀念換來的東西**。18 頁。

**③ 節名「現在會咬到你的」站主自己讀不懂。** 它是英文 "come back to bite you" 的直譯，中文沒有這個用法。查出是 09-06 第 4／5／7 波各生一個、複製到三頁的句型：`claude-code`「現在會咬到你的」、`ai-agent-safety`「現在會打到你的」、`anthropic-business`「現在還打得到你的商業風險」。

**為什麼沒人擋下來（本條是給流程的）：** 翻健檢紀錄發現**冷讀者三次都放行，還逐字寫「也很清楚」**——冷讀者是 LLM 扮的，讀到中文直譯會自動對回英文原 idiom，所以真的「懂」；真人沒有那條回路。`check_reader_language.py` 也接不到，它比對禁詞清單，翻譯腔列不成清單。**這一類只有真人抓得到。**

**處置：** 三個節名改為「現在還沒修好的」「現在還擋不住的攻擊」「還沒過去的風險」，全庫回掃 43 處（wiki 36＋規則檔 7），`log.md` 與 `docs/page-audits/` 不動。查證後確認**沒有任何錨點或腳本依賴這三個節名**，registry 也沒凍結——措辭從頭到尾是慣例不是契約。依使用者「可以變開放用語不要指定格式嗎」，規則檔改用「角色＋欄位形狀」指涉，措辭放括號當「現名」；新增格式規則「節名是讀者的話，不是機械契約」，明訂只有三種情況凍結（錨點入邊／腳本 grep／registry 登記）。

**同日追加裁決（使用者：「你一定要固定用語嗎 我不太想限制」）：** 上面那條規則原本還有一段規範性文字（「節名用讀者聽得懂的中文，不套英文 idiom 的直譯」），**已刪除**。理由是條文自己就寫著「這一類只有真人抓得到」——**檢查不到的規則是裝飾品**，而既有的「無維運術語洩漏」規則與讀者語言閘都在場、兩個都沒擋住這個詞，再加一條講品味的條文下次一樣擋不住。規則只留機械的那半：**節名預設可以改，只有三種情況凍結**（錨點入邊／腳本 grep／registry 登記）。教訓（冷讀者是 LLM 扮的、對翻譯腔沒有抵抗力）留在本 log，不進規則檔。節名本身也只是「現況」不是「規定」——想改隨時改，規則檔不會擋。

**同日續：開發實務標籤逐頁裁決（18 → 14 頁），入口表同步收斂。** 使用者逐頁裁下：模型實體頁**一律不列**（拿掉 `fable-5`、`opus-5`——選型的家是 [[topics/model-comparison]]＋[[topics/model-task-leaderboard]]；否則會出現「fable-5 在、而 Claude Code 預設模型 sonnet-5 不在」這種只因 index 括號寫沒寫到的不一致）、`competitor-landscape` 不列（競品雷達＝市場在幹嘛）、`anthropic-business` 不列（估值／IPO／營收，跟寫 code 的距離同 market-signals）。`claude-code` 與 `community-pattern-trends` 留（前者的「現在還沒修好的」結論層是 09-06 才長出來的，09-03 把它擋掉時那層還不存在）。
...
## 2026-09-13 蒸餾封存執行（09-12 lint 3h 提案，使用者已裁決）

派四位記者執行 18 個提案時段。**已蒸 13 個時段、判定不蒸 3 個、本輪不處理 2 個**；新建 archive 子頁 6 個，沿用既有 1 個。首輪派工曾因 session 配額上限（台北 16:00 重置）全數中止、工作樹無殘留，16:30 後重派完成。

**已蒸（13 個時段／7 頁）**
- 人物：`entities/boris-cherny` 2026-04＋2026-05（274→261 行，新建 archive 74 行）、`entities/dario-amodei` 2026-05＋2026-06（141→135 行，新建 archive 53 行）
- 商業：`topics/enterprise-tool-tracker` 2026-04＋2026-05（365→355）、`topics/enterprise-cost-management` 2026-05＋2026-06（303→261）、`entities/pricing` 事故區與定價促銷區 2026-05（735→717，沿用既有 `pricing-archive`）、`topics/ai-talent-flow` 2026-06（145→134）、`entities/google-investment` 2026-04＋2026-05（125→124）
- 功能：`topics/official-community-gap` 2026-05＋2026-06（242→235 行，新建 archive 58 行）
- 回掃：8＋4 個時段逐一 grep 全庫 `[[頁#條目標題]]`，**全部零命中，無連結需要改指**（命中的只有 append-only 的 `log.md` 與會重新生成的建置產物）

**判定不蒸（3 個時段）**
- `topics/recursive-self-improvement` 2026-06：記者回報命中例外 2 與例外 3。**主編核對後更正其例外 2 的理由**——記者寫「`## 摘要` 第三段有『見下方時序』指向 06-22 CNA 條目」，開檔核對**該句不存在**；但結論仍成立且理由更強：該頁摘要前三段（06-04《When AI Builds Itself》報告、06-22 五眼聯盟聲明）講的全是 2026-06 的事，那一段本來就是摘要的細節家，搬走等於抽掉摘要的地基。例外 3（本週 09-09～09-12 Coxon／Hubinger／Benton／Engels 一系列離職示警正在重燃同一條敘事，而 06-04 是起源事件）獨立成立。**本輪未建立 `recursive-self-improvement-archive.md`**
- `topics/code-quality-decline` 2026-06：**三條例外全部未命中**（錨點入邊 0；表上兩列的細節家分別在已封存的 `### 2026-03～04` 與獨立的「## Token 消耗異常訊號群」覆寫式表；2026-06 的兩則條目與正在重燃的兩條線是不同機制），但記者查到**頁面專屬登記擋下**：`.claude/reporter-rules/community/pages.md` 第 202-204 行明寫「2026-06 於 2026-10 起達門檻」。今天 09-13 未到。記者選擇以頁面專屬登記為準，判斷正確
  - ⚠️ **順帶暴露一個口徑不一致**：通用算法（本月序 − 該月序 ≥ 3）此刻已達 3、09-12 lint 據此把這格列進候選，但頁面專屬登記寫的是 10 月，兩者差一個月。本輪依嚴者（頁面登記）執行；**待裁示：是要改頁面登記對齊通用算法，還是在通用算法加一句「頁面另有登記者以登記為準」**
- `topics/safety-china-trust-dispute` 2026-06：該時段僅 1 條，依裁決等 07 月一起蒸，本輪不處理（原提案即如此標注）

**主編收尾**：`wiki/index.md` 的 `enterprise-tool-tracker` 列因新增「↳ 子故事」投影而超 120 字元上限（記者依規則不碰 index，留交主編），已縮短鉤子文案至 96 字元。`gen_wiki_frontmatter.py` 全庫重跑，6 個新 archive 頁的上層、領域繼承與 index 投影全部就緒。

**檢查**：713 個測試案例全過；wiki 新鮮度 85 頁通過；字元上限、讀者語言、懸置標記、子故事階層四閘全綠。

**記者自陳的一次失誤（如實記錄）**：人物記者開工前誤在未編輯任何內容的狀態下整庫跑了一次 `gen_wiki_frontmatter.py`，波及 76 個不相關頁面的 `days_since_news` 欄位，已用 `git show HEAD:<path>` 逐一還原後才開始正式編輯（未使用被規則禁止的 `git checkout`／`reset`）。

## 2026-09-13 Query：`llm-wiki-pattern` 頁「缺的是查詢層」已過時（使用者指出）
...
## 2026-09-20 Query：頁面健檢第 14 波 official-community-gap 定稿

- **查證**：`gh issue view -R anthropics/claude-code` 掃頁內全部 issue、官方 CHANGELOG 原檔、code.claude.com 與 platform.claude.com 文件。AGENTS.md 於 v2.1.277（2026-09-18）原生支援；2026-08-17 官方一次關閉 #6235／#24798／#47023／#24316／#29006（#28322 於 08-19）；手機推播 2026 第 16 週即有；`/usage` 方案用量拆解＋狀態列 `rate_limits`＋VS Code 70% 警示；Dynamic workflows 開放所有付費方案；Claude Code Projects public beta；Managed Agents 仍 beta、Dreaming 為其下須申請的研究預覽；Cowork 網頁／行動版官方日期 07-07、用詞 beta access。
- **根因**：[[topics/official-community-gap]]「官方對應」欄沒有人定期對官方一手，13 列中 8 列低估官方，且全庫零矛盾、庫內自洽檢查測不到。
...
## 2026-09-20 待裁示清理（使用者裁示「都照做」）

11 項 lint 待裁示逐項處置，**已結 10 項、1 項需使用者裁決**。

- **6f 指路失效**：指向已廢除的 `review-commands.md`「同步配對註冊表」→ 改指 `.claude/review-registry.json` 的 `sync_pairs`，並登記 sync_pair 看守（原本無人看守，才會指到廢節還沒人發現）。
- **6e 未註冊 slug**：`blogroll`／`official` 不在 registry（正確為 `blog`／`official-docs`），3 筆歸因改正，其中 1 筆是本輪自己寫錯的。
- **5g 週更快照頁誤觸**：不調全域門檻（`STALE_DAYS` 一動，全庫真停滯也被放過），改成頁面層宣告 `**新鮮度豁免：**<理由>`，signal 給自己一格 `🗓️ 非新聞驅動`、理由寫進 `staleness_exempt` 欄。豁免不等於隱藏。
- **5k 表超載**：51/50，無 🌙 逾 90 天可自動退場，屬滿載讓位。汰最舊的 🌙（Boris Cherny「coding is solved」，88 天，事實在同頁長期議題節與人物頁都有家），現 50/50。
- **skill-interest 快照腳本兩項**：callout 單則 211 字元 → 拆四條子條列（最長 68）；「125 列工具目錄」寫死 → 改每次現算（實際 157），抄不到時寫不帶數字的說法。
- **榜單汰換 → 取消**：Search Arena／LMArena 的「連續抓不到」根因是 `arena.ai` 改版搬家，不是榜死掉。新位址 `/leaderboard/text`（改名 Text Arena）與 `/leaderboard/search`，兩榜已接回直抓。順帶更正一筆事實：上週記的「GPT-5.6 Sol 第三」為媒體轉述之誤，實際前三名全是 Claude（但差 4 分、信賴區間 ±4~5，實質分不出高下，已於頁面寫明不可當選型依據）。⚠️ Search Arena 榜頁自標 08-24、已 27 天未更新——是榜自己沒動。
- **併頁候選 → 不併**：`anthropic-agent-stack` 答「官方出了哪八塊積木」（🛠️ 功能記者、吃官方文件），`community-large-codebase-workflow` 答「社群在大型 repo 怎麼做」（🌐 社群記者、週更沉澱）；問題、進料、單一維護者都不同，併頁會讓同一頁出現兩個記者。已登記 `data/graph_gap_ignore.json`。
- **community-tech-tools 汰除欠帳**：157→152。13 列達標者逐一用 `news_mentions.py --since 4w` 查後續，汰 5、保留 2（有後續）、規則豁免 6。連帶清掉一條指向已刪列的孤兒註腳。
- **蒸餾 ~38 個時段**：實蒸 22、不蒸 3（皆寫明擋下來的例外條）、已完成 13。新建 10 個 archive 子頁，回掃入邊皆 0、無斷鏈。安全政策記者正確拒絕 lint 的「06 併 07 月」建議（07 月未達 ≥3 個月門檻），契約優先於建議。
- **6d 過期規則 42 節**：逐節核實得 27 仍有效／15 需更新／0 可刪。15 節共同病是寫死數字與指路漂移（檢查類數 4→6、角色檔八→九、回報欄八→11、6f→6e 編號、people 欄位語法、`.tab-row`／`.entity-card` 兩組類名全庫從未存在）。會漂的數字一律改成指回權威來源。

**順帶抓到三個根因，都不是原本要查的東西：**
- **5c 積壓的根因不是標記門檻鬆**。查了近 14 天 65 筆新標記的來源與類型——「等官方回應」型只有 8 筆且都年輕（中位數 8 天），不是積壓來源。真因是兩個數字從沒被並排比過：**複查日中位數 30 天，但 Lane B 佇列排空要 92 天**，每一筆都必然在輪到它之前先逾期，「逾期數」因此只是佇列長度的別名，照它調額度會調錯方向。已讓 `check_pending_markers --queue` 每輪印出這組對照與兩個可行解。
- **子故事投影不遞迴**（`gen_wiki_frontmatter.py` ↔ `check_hierarchy.py`）：巢狀子頁（子頁自己又長出子頁）在 index 上無列可掛，投影端掃不到、檢查端卻要求它，必然驗不過。這輪 managed-agents 長出 archive 時第一次踩到。兩端改同源並登記 sync_pair。
- **字元上限閘把機器投影算進人寫的預算**：每多一個 archive 子頁，就會要求另一個人去砍他沒寫過的鉤子。`↳ 子故事：` 段改為不計入字數。

**⏳ 仍待使用者裁決 1 項（不自行決定）**：`web-reader-design.md` 禁止清單寫「❌ gradient、❌ box-shadow（除 focus ring）」，但實作有 `.search-modal` 的 `box-shadow: 0 24px 64px`（自 05-15）與三處 `repeating-linear-gradient` 畫虛線分隔（自 05-02）。6d 盤點建議「加註為已知例外」，但該節同時記載 **2026-07-26 使用者裁決維持全面禁用、後續 review 不需再提**——寫成例外等於推翻那次裁決，故不自行處理。要嘛認列例外、要嘛把實作改回來，請裁示。

## 2026-09-20 5c 逾期待查證清算：109 筆清零（使用者裁示「先清 0」）

**為什麼清 0 而不是改指標**：複查日中位數 30 天，但 Lane B 佇列排空要 92 天——每一筆都必然在輪到它之前先逾期，「逾期數」因此只是佇列長度的別名。使用者裁示直接把隊伍清空：結構性落差消失後，之後再出現逾期才真的代表有人沒做，不必重新定義「逾期」的語意。清零後複查日 30 天 > 排空 11 天，該落差警告已不觸發。

- **分工與結果**：六記者依 5c 契約分頭查證，109 筆（Lane A 4／Lane B 105）全數結案——安全政策 28、功能 28、商業 21、社群 16、模型 11、人物 5。處置以「查到官方一手來源、改寫為事實」為大宗；查不到者依契約改 🔎 查無官方（定義是「已查官方、確認未載」，不是「沒查」）；Lane A 探針假命中者退回 Lane B 重查。
- **順帶查實並更正的事實錯誤**：
  - [[entities/mythos]]「UpToDate 深化合作」實為 Epic 自家與 Wolters Kluwer 的產品整合，**與 Anthropic 無關**——原條目把兩則獨立消息捆在一起。
  - [[entities/jensen-huang]]「後悔」方向查反了：2026-08-26 財報電話會議原話為「I think the only regret that I have is that I didn't invest more and sooner」，是**嫌投太少、太晚**，不是嫌投太多。[[index]] 與 [[overview]] 的摘要句一併上修。
  - [[entities/tom-blomfield]] 加入 Anthropic 由傳聞轉為已確認（本人 X ＋ LinkedIn），職稱 Member of Technical Staff、compute 團隊。
  - [[topics/enterprise-tool-tracker]] 原判 JPMorgan 與 Goldman 帳號限制同源，實為不同根因（出口管制 vs 區域存取政策）。
- **兩個機械閘的處理**：過程中有 agent 跑了全庫 `check_cell_limits.py --rebuild`，把其他並行 agent 新增的超限條列一起吸進基線。已還原基線重量——全庫 928 vs 清零前 963（**淨 −35，且無任何一頁高於清零前**），確認 59 筆「新增」幾乎全是改寫既有長條列造成的指紋變動而非新腫；真正變多的六頁 21 筆已逐條拆成巢狀子條列（事實未改）後，才重收基線並附理由。懸置標記基線亦重建（106 筆）。
...
## 2026-09-20 Query：週報選材整體 review（使用者提問）

依 `REVIEW-PRINCIPLES.md` 判定適用 A／B／D／E 區（C 不適用，無視覺成品）。量測範圍：`weekly/2026-W30`～`W38` 九期。

- **P0｜回收判定會判錯**：探針用撰稿者自造的詞，日報用另一套。W37「配額換軌之後」探針（週配額／weekly limit／17%／撞上限）立案當週即全數零命中；W38 回收據此判「零實測回報、續盯」，實際當週有 5 則相關條目（09-15「Max 5x 用戶質疑用量縮減幅度不只官方所稱的 25%」等），應走判準第二支結案。九期 150 個探針中 21% 立案當週即零命中；整條全滅的 3 條後來全數以「零命中」收場。
- **P1｜預告產出率低且惡化**：57 列回收中 42%「什麼都沒發生」；近三期 19 列僅 1 列被判準切中、68% 零動靜。主因是偏好「官方會不會修／回應」型，該型基本率本來就是沒動靜。
- **P1｜讀者興趣優先權形同虛設**：九期僅 W35 命中一次。W37（GPT 對照，當週提及 12 次）與 W38（AGENTS.md 出貨＝跨工具格式規約，正是 08-08 那條在等的第三節點，當週提及 8 次）皆有訊號未對上——比對的是筆記字面而非意思。
- **P2｜深挖題材集中**：成本 5 期、安全 2–3 期、規則寫法 2 期；新功能怎麼用／選型／工作流 0 期。記為觀察，未立規則。
- **P2｜涵蓋窗缺口為單次失誤**：W30–W37 動筆當日日報皆有用到；W38 漏 09-19 是開工前未 pull，非結構問題。
...
- 來源日報：[[news/2026-09-25]]
- 更新頁面：wiki/entities/claude-code.md、wiki/entities/claude-skills.md、wiki/entities/cowork.md、wiki/entities/claude-docs.md、wiki/entities/claude-slides.md、wiki/entities/claude-science.md、wiki/topics/claude-code-experimental.md、wiki/topics/anthropic-business.md、wiki/topics/competitor-landscape.md、wiki/topics/anthropic-government-policy.md、wiki/topics/ai-agent-safety.md、wiki/topics/community-tech-patterns.md、wiki/feature-radar.md
- 新增頁面：無
- 摘要：Anthropic 與 Akamai 簽 7 年 116 億美元運算合約；白宮令 OpenAI／Anthropic 暫緩向英國測試機構釋出新模型；Claude 發現的類 CRISPR 酶系統延燒多家媒體（New Scientist 審慎、Gizmodo 指 Anthropic 無法說明能力範圍）；Opus 5.5 上線後社群持續討論是否還需要 Fable；Claude Code v2.1.282（maxProseWidth 設定）與多筆官方文件異動（Cowork 擴及所有對話、桌面版 skills/plugins 讀取規則）
- 呈現品質：全部通過（功能、社群記者過程中曾觸發字元上限／讀者語言閘，已修正重跑至綠燈）
- 品質備註：[安全政策] 全庫懸置標記舊語法存量 42→43（+1），經核對非該記者所動頁面/行號所致，未查明是哪位記者所留，待 `/wiki-lint` 複查
